...
Anima is native in ComfyUI core, with official templates and no custom nodes for text-to-image. Like the suite's other DiT models, it is **not a bundled checkpoint**. It needs three files and three loaders `[official-via-docs]`:
...
- **Native 1536² is a real capability, not an upscale.** It is a concrete advantage over SDXL-anime, where 1024-area is the ceiling before duplication artefacts. Prefer generating large over generating small and climbing (§9). Outside the band, expect duplicated subjects and stretched anatomy above, and mush below.
...
- **8 GB is a working floor, 12 GB comfortable.** That rests on one report on one AMD card `[flagged — re-verify]`. `u/Dependent_Quit_3730`, on an 8 GB RX6600 with 16 GB RAM, says Anima is the only model that loads at all. The cost is speed. `u/Bokayoteamo`, on the same GPU via ROCm: *"1024x1024 30 steps takes around 8-15 mins."*
...
- **Quantisation.** `silveroxides/Anima-Quantized` is often cited, but it is largely preview2-era *resized turbo-distill LoRAs* rather than a quantisation of Base v1.0 `[community — re-verify]`. At 4.18 GB, most people never need one.
- **AMD memory creep** `[flagged — re-verify]`. Two independent reports back this up: generation time climbing from ~20 s to over a minute after 20–30 images, cleared only by a full restart (`u/Greyblades2`); and VRAM+RAM exhaustion on an Illustrious→Anima refine chain without a VRAM-management node (`u/Alekite`, R9700 32 GB). The cause is not pinned down between Anima and ROCm. On AMD, put a VRAM-cleanup node in the graph before concluding the model is broken.
...
- Repackage: **`circlestone-labs/Anima-Base-v1.0-Diffusers`**, whose `modular_model_index.json` declares `"_class_name": "AnimaModularPipeline"`, `"_blocks_class_name": "AnimaAutoBlocks"` and `"_diffusers_version": "0.39.0.dev0"`. That is **a dev build, not a released diffusers**. Default is 1024×1024, with dimensions in multiples of 16.
...
- **The repo ships a `t5_tokenizer/` folder, and it is not a mistake.** `modular_model_index.json` registers it as `["transformers", "T5Tokenizer"]`, and the adapter config carries `"target_vocab_size": 32128`, the T5 vocabulary. **No T5 encoder weights are loaded anywhere.** The T5 tokenizer exists only to carry prompt-weight multipliers into the LLM adapter. See SKILL.md's *one rule* for why this is the mechanism behind Anima's higher-than-SDXL weights.
...
- **No cross-compatibility with SDXL, Illustrious, NoobAI or Pony LoRAs.** The architecture, latent space and encoder are all different. An SDXL LoRA either fails to load or no-ops, and there is no conversion path. This is the most common false expectation Illustrious migrants carry over.
...
- **Whether Anima LoRAs load on the 2.9B/3.8B forks is unanswered** `[flagged — re-verify]`. `u/Neonsea1234` asked and got no reply. Assume not. The forks add extra layers.
...
Anima inherits **Cosmos-Predict2's reference-conditioning path**. That is why its editing tooling looks nothing like SDXL's. Three routes exist, and none is finished.

### (a) Anima-LLLite — kohya-ss's control family, and the usable route

**Native in ComfyUI core.** No custom node is needed for the released weights (kohya's node is only for the unreleased v3 below). Weights go in `ComfyUI/models/model_patches/`, loaded by **`ModelPatchLoader`** and applied by **`AnimaLLLiteApply`** (`model`, `model_patch`, `image`, `strength`, `start_percent`, `end_percent`, optional `mask`; category `model_patches/anima`).
...
**Published control types**: `lineart`, `depth`, `pose`, `scribble` (Preview3-era), `inpainting-v1`/`-v2`, `any-test-like-1`/`-v2`. **There are no canny or HED weights.** The "any control" template uses the any-test-like weights instead, trained on *"Lineart / scribble / grayscale, heavily augmented"*, and these absorb canny-ish and HED-ish inputs in practice.
...
1. **Base gen.** Generate at target size directly, up to 1536². Judge composition, and re-roll seeds freely before touching anything else.
2. **One hires pass, optional.** Use a single second sampler at **denoise 0.3–0.45**, or a latent/pixel upscale then one re-sample at **0.25–0.35** `[community — suite-wide refine bands]`.
...
> **The trap: do not chain two KSamplers with hi-res fixes on both sides.** `u/AssistanceSouth9359`: *"anima is pretty good at creating close up shots > Halfbody shots > 3/4 body shots but it loses a lot of details or the image gets fried (weird, soft black spots all over the image) when i try to use 2 ksamplers, pre and post hi res fixes."* Nobody in-thread solved it. The mechanism: each pass re-noises an already-denoised latent. A 2B backbone compounds the resulting high-frequency artefacts instead of resolving them, the way a larger model would. **One hires pass, or none.**
>
> A second rule sits in the same quote: detail quality tracks how many pixels the face occupies. Close-up beats half-body, which beats ¾-body, which beats full-body. A full-body shot with a good face needs a detailer pass.

...
Anima is CircleStone Labs' open-weights **anime and illustration** model, *"created via a collaboration between CircleStone Labs and Comfy Org"* `[official]`. It is a **flow-matching DiT** of **~2B parameters** derived from **NVIDIA's Cosmos-Predict2-2B-Text2Image** — `CosmosTransformer3DModel` in CircleStone's own diffusers repo, `class Anima(MiniTrainDIT)` in ComfyUI core, with the transformer alone at 1.96B in bf16 — conditioned by a **Qwen3-0.6B *base*** text encoder routed through a dedicated **LLM adapter**, and decoded by the **Qwen-Image VAE**. Trained on Danbooru-style tags, natural-language captions and hybrids; knowledge cut-off **September 2025**; resolution band **512²–1536²**. Weights landed **14 May 2026** (announced 15 May) after a January 2026 preview line. Licence: **CircleStone Labs Non-Commercial License v1.2** *plus* inherited **NVIDIA Open Model License** terms via the Cosmos derivation — two licences, both binding, and **generated images are carved out of both**. Comfy Org's contribution is reported as a ~$1M open-model grant plus day-one ComfyUI support `[community — Civitai 26217, HF disc. 185]`.

**The defining trait: Anima has an LLM text encoder and prompts like a booru model.** Danbooru tags in a trained order, `score_7`/`masterpiece` quality ladders, `safe`/`explicit` rating tokens, an `@artist` prefix, ComfyUI attention weighting — the whole SDXL-anime dialect — driven through a small LLM rather than dual CLIP. That combination is not supposed to exist (see *The one rule*), and it is why a reader arriving from Illustrious feels at home while one arriving from Z-Image writes prompts that quietly under-perform. The second defining fact is a stated non-goal: **"The model doesn't do realism well. This is intended."** Anima is a companion to the suite's photoreal models, not a competitor.
...
> **The Turbo-vs-Aesthetic default is contested** `[contested]`. CircleStone recommend Turbo — *"only slightly worse than Anima-Aesthetic, while being very fast to generate"* — while `u/Time-Teaching1926` reports its *"quality, styles, stability and even sometimes prompt Adherence isn't that great."* A/B both before committing a pipeline.
>
> **A community checkpoint already out-downloads the official base** — MiaoMiao Harem, ~199k against ~190k `[community — Civitai API, 2026-08-22]`. Treat Anima as this suite treats SDXL: the base is the training substrate; most people generate on a finetune. Roster: [`references/setup-and-workflows.md`](references/setup-and-workflows.md) §11.

---

## The one rule that changes everything

**Prompt Anima in weighted Danbooru tags, in its trained tag order, with an `@` in front of every artist — and push the weights far past what SDXL taught you.** The card's worked example is `(chibi:2)`, and it says in as many words that a term *"needs a weight higher than typically used for SDXL."* `[official]` That is not folklore; it falls out of how weighting is wired (below). A reader carrying SDXL's 1.05–1.3 habit will under-weight every emphasis they write and conclude, wrongly, that Anima ignores weighting.
...
Three mechanics follow, all silent failures — wrong output, never an error:
...
3. **Quality tags are per-variant.** Aesthetic had them stripped from its captions — feeding them back pushes it *"too hard into slop territory."* Drop **both** ladders there, not just `score_*`.
...
**Verbatim from the stock `image_anima_base_v1` template: 30 steps, CFG 4, sampler `euler`, scheduler `simple`, 1024×1024** `[official]`. That is the runnable default and the answer to "which scheduler" — **`simple`**.
...
- **`beta57` is not stock.** The authors recommend it for a painterly look — *"it puts more emphasis on low-noise timesteps"* — but it ships in the **RES4LYF** pack (`ComfyUI-RES4LYF`, via ComfyUI Manager) and will not appear in the dropdown until installed.
...
CircleStone publish no inference VRAM figure. At **4.18 GB** plus a 0.6B encoder and the VAE, Anima runs when nothing else will — it is reportedly the only model that loads at all on an 8 GB RX6600, at 8–15 min per 1024²/30-step image. Treat **8 GB as a floor, 12 GB as comfortable**, and re-verify: that floor is one report on one AMD card `[flagged — re-verify]`. Detail and the quantisation caveat: [`references/setup-and-workflows.md`](references/setup-and-workflows.md) §5.

### diffusers

`circlestone-labs/Anima-Base-v1.0-Diffusers` declares **`AnimaModularPipeline`** with `AnimaAutoBlocks` and `"_diffusers_version": "0.39.0.dev0"` — a dev build, not a release. (`AnimaImagePipeline` is **DiffSynth-Studio's** class, not diffusers'.) Default 1024×1024.
...
1. **Artist tags with the `@` prefix.** The deepest vocabulary and the real differentiator: ThetaCursed's **Style Explorer** indexes **42k+ artist styles for Anima Base** against 16k+ for Illustrious/NoobAI `[community — ThetaCursed]`. Stack two or three to blend, and weight them (`(@artist:1.6)`) rather than repeating them.
2. **Quality and year tags.** The `masterpiece … worst quality` and PonyV7-derived `score_9 … score_1` ladders are **two independent systems, usable alone, together or not at all**. Year tags (`year 2025`, `newest`, `old`) are a separate, unusually strong style axis — `u/RevolutionaryWater31`: *"the year tags — this has very strong influence on the generated image."*
...
> **It is not shipped yet** `[pending release]`. Both halves are open, unmerged **drafts** — `kohya-ss/sd-scripts` **PR #2413** (model) and `kohya-ss/ComfyUI-Anima-LLLite` **PR #10**, branch `feat-v3-semantic-trunk` (node) — with kohya's status *"merging into main is not decided yet."* The weight lives at **`kohya-ss/Anima-LLLite`**, *not* the Comfy-Org repackage.
...
| Symptom | Cause | Fix |
|---|---|---|
| Artist style barely registers | The artist tag was written without `@`, so it tokenises as ordinary general tags instead of hitting the trained artist vocabulary — the image still renders, so nothing errors | Prefix every artist with `@`; weight it `(@artist:1.6)` if still weak |
| Weighted terms seem to do nothing | Weights are a flat multiplicative scale on post-adapter embeddings, not CLIP's interpolation toward the mean — SDXL's 1.05–1.3 barely moves the conditioning | Start at 1.5 and step by 0.25; Anima tolerates `(at night:2.0)` without breaking |
| Aesthetic output looks over-cooked, "sloppy" | Quality tags fed to Aesthetic, whose captions had them stripped — the tokens land as an out-of-distribution style push | Drop **both** ladders (`masterpiece`/`best quality` *and* `score_*`) from positive and negative |
...
| Image "fried" — soft black blotches after upscaling | Two chained KSamplers each re-noise an already-denoised latent; a 2B backbone compounds rather than resolves the artefacts | One hires pass at denoise ≤0.45, or generate natively up to 1536² |
...
**Anima vs an SDXL anime finetune vs Z-Image, in one paragraph.** Selling *images*? Anima is fine — that is the licence's explicit carve-out, and the suite's older "Anima loses on commercial grounds" shorthand is simply wrong. Shipping *the model* — an API, a plugin, a game bundling weights? An Illustrious/NoobAI/Pony finetune under [`sdxl`](../sdxl/), which also keeps the deepest ControlNet and identity stack. Otherwise Anima wins on prompt comprehension, artist vocabulary, native 1536² and cheap training, and the community reads it as Illustrious's successor (`u/Massive-One-3543`: *"most people use Anima — just as they used Illustrious before that, and Pony prior to that"*). [`z-image`](../z-image/) is not in this contest — a photoreal-leaning sentence-prompted generalist, so booru anime fights both its dialect and its aesthetic.
...
**Content terms.** §4(a) bars generating *"unlawful content, including child sexual abuse material, or non-consensual intimate images"*, and — load-bearing for character work — bars use infringing *"any third party's legal rights, including rights of publicity or 'digital replica' rights."* `[official]` Real-person likeness is therefore constrained by **this licence as well as** platform rules and law; [`character-lora-training`](../character-lora-training/) owns the practical gate. No general adult-content prohibition beyond that — consistent with `safe`/`sensitive`/`nsfw`/`explicit` shipping as trained rating tokens rather than a refusal layer.

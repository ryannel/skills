...
1. **Qwen-Image-Edit factory (best).** Feed the anchor + ~20–60 edit prompts: the 8-point rotation, expressions, outfits, lighting setups, settings. Generate ~60, **curate the best ~30** — cut anything where the face drifted. 2511 (Dec 2025) is the release character-LoRA builders reach for when consistency matters `[community — single report]` — and it is not "one version behind": Qwen-Image-2.0 (Feb 2026) is API-only, so 2511 is the newest edit model you can run locally `[official — QwenLM, 2026-08-23]`. Setup below.

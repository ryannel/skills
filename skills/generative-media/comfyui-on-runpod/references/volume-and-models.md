...
**Budget for the weights existing twice.** Trainers read the base model in diffusers format out of the HF cache. ComfyUI loads a single-file `.safetensors` from `models/diffusion_models/`. For a Z-Image-class model, that is roughly 20 GB stored twice. [`qwen-image`](../../qwen-image/) is the heavy case: an fp8 inference set is ~30 GB per Edit generation (20.4 GB DiT, 9.4 GB encoder, 0.25 GB VAE), and training needs the 40.9 GB bf16 DiT plus the 16.6 GB bf16 encoder on top, because musubi-tuner rejects the fp8 repacks. This is a format difference, not waste you can deduplicate. Size the volume for both copies rather than trying to make one file serve both jobs.

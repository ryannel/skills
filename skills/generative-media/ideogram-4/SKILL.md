...
description: >
  ...
  (Quality 48 / Default 20 / Turbo 12) and resolution; setting up the open weights (nf4/fp8 for diffusers,
  fp8_scaled / nvfp4_mixed / int8_convrot for ComfyUI, VRAM, HF gating, ComfyUI nodes and file layout,
  DualModelGuider, the TurboTime speed LoRA, the gemma4 gotcha); training a LoRA on it (ai-toolkit,
  musubi-tuner, fal; OneTrainer's output does not load; style LoRAs work, character LoRAs are
  undemonstrated); using it as the typography pass in a mixed-model pipeline; routing
  ...
  consistent characters on open weights: with no identity adapter, edit variant or published character
  LoRA, identity comes from a locked half-canvas workflow. Use this for any question about Ideogram 4.
  Choosing between models, or which skills and install commands a job needs, is
...
**This skill is about running the open weights.** That means the gated weights, in whichever quant fits your card, on your own GPU or a rented cloud GPU, via **ComfyUI**, the diffusers **`Ideogram4Pipeline`**, or the **`run_inference.py`** CLI. That path gives you full control over every sampler knob, and it is the path the rest of this guide assumes.
...
A fourth speed point exists as a community LoRA rather than a preset. ostris's **TurboTime** LoRA runs the model in as few as 2 steps with no CFG and no unconditional model `[community — ostris, HF + Civitai]`. It changes the graph, not just the step count, so it is covered under *ComfyUI* below.
...
- **Self-hosted open weights (gating):** the weights are gated on Hugging Face. Accept the licence and authenticate (`hf auth login` / `HF_TOKEN`), or the downloads 404. **The quants come in two sets, for two runtimes.** For diffusers and the CLI, `ideogram-ai` publishes two: **`nf4`** is bitsandbytes 4-bit, **CUDA-only**, and fits a 24 GB GPU; **`fp8`** is weight-only float8 and runs on **any hardware**, with activations staying bf16. For ComfyUI, `Comfy-Org/Ideogram-4` publishes three, each as a conditional + unconditional pair: **`fp8_scaled`** (the template default), **`nvfp4_mixed`** (the 4-bit file) and **`int8_convrot`** (the newest) `[official — Comfy-Org/Ideogram-4 repo, 2026-09-09]`. No quality, speed or VRAM comparison between the three has been published; `references/setup-and-workflows.md` §4 carries that open flag. diffusers exposes **`Ideogram4Pipeline`**, and the repo ships a `run_inference.py` CLI. All three stacks run identically on a **rented cloud GPU**, which is the usual way to reach 24 GB+ for the larger quants and 2K renders. Quick start, sampler presets and VRAM notes: **`references/setup-and-workflows.md`**.
...
**LoRA training does exist — as a style ecosystem, not a character one.** Three trainers emit a file the open weights load: `ostris/ai-toolkit` lists `ideogram-4-fp8`, `kohya-ss/musubi-tuner` has first-class support (`networks.lora_ideogram4`, since 2026-07-04), and fal's live **Ideogram V4 LoRA Trainer** emits a ComfyUI-format file alongside its own. A fourth, OneTrainer, has an extension whose output ComfyUI cannot load `[community — ComfyUI issue #14477]`. One named hyperparameter recipe exists, from a single author `[community — estylon, Substack; single report]`. **36 LoRAs are published on Civitai, one of them character-tagged**, the rest style work `[community — Civitai API, queried 2026-09-09]`. A *trained* likeness is therefore still exploratory here: one uploader's tag, with nothing published about how it was made or how well it holds. For one you can rely on today, build it in [`flux-2`](../flux-2/), [`z-image`](../z-image/) or [`sdxl`](../sdxl/). A likeness you only need to carry across a handful of generations needs no training at all. That is the next section. Training details are in `references/lora-training.md`, plus [`character-lora-training`](../character-lora-training/) for the craft that transfers across models.
...
**What's unusual: two diffusion models load** and combine through a **`DualModelGuider`**. This is the model's dual-branch (asymmetric) CFG, a conditional transformer plus a separate unconditional one. It is not a negative-prompt string. Stock template defaults: sampler **`euler`**, **`Ideogram4Scheduler`** at **20 steps** (the Default tier: a `CustomCombo` switches between Quality 48 / Default 20 / Turbo 12), `DualModelGuider` guidance **7**, latent **1024×1024**. The two `fp8_scaled` files are one of **three quant pairs** in `Comfy-Org/Ideogram-4`; `nvfp4_mixed` and `int8_convrot` swap in by changing both `UNETLoader` widgets (`references/setup-and-workflows.md` §4).

**The one thing that removes the second model is a LoRA.** ostris's **TurboTime** LoRA claims 2-step sampling with no CFG and no unconditional model `[community — ostris, HF + Civitai]`. With it loaded, the second `UNETLoader` and the `DualModelGuider` come out of the graph, because there is no unconditional branch left to guide against. That is the same trade as Wan's lightx2v LoRAs: guidance baked into the weights, speed bought with some loss of range. It is one author's release, and nobody has reproduced it independently yet; the open question is listed under *Contested / unresolved points* below.
...
The full node table, the three quant pairs, VRAM reports, the TurboTime graph change, and GGUF status are in **`references/setup-and-workflows.md` §4**.
...
|---|---|---|
| In-image typography, layout, design | **The leader** — JSON captions, `bbox` layout, text layers, transparency | — (this is why you're here). [`qwen-image`](../qwen-image/) for two jobs this model does not do: editing text already in an image while keeping its font, and bilingual Chinese/English rendering under Apache-2.0 |
| Consistent characters | **Reachable, by a workflow rather than a tool.** There are no identity adapters, no edit variant, and one character-tagged LoRA of 36 with no recipe behind it — and none of that settles the question: a locked half-canvas carries a likeness into a new scene (*Consistent characters without an adapter*, above) `[community — reality_comes, 402 pts]` | [`flux-2`](../flux-2/) (multi-ref + PuLID), [`z-image`](../z-image/) or [`sdxl`](../sdxl/) (character LoRA) when the likeness must be exact, measured, or reused at scale; or hosted Character Reference |
| **Style LoRAs** | **Supported** — ai-toolkit, musubi-tuner and fal's trainer all emit ComfyUI-loadable weights (OneTrainer's do not load); one named starting recipe `[community — estylon, Substack; single report]`; 36 published on Civitai (2026-09-09) | Fine to train here. [`krea-2`](../krea-2/) if you want the official train-Raw/run-Turbo doctrine |
| **Character LoRAs** | Trainable but essentially **undemonstrated** — the path exists, the ecosystem doesn't: one character-tagged entry of 36, no write-up. One cross-model comparison exists, rating Ideogram 4 *above* Krea 2 and Z-Image at learning tattoos `[community — Any_Tea_3499; single report]`, recorded in [`krea-2`](../krea-2/references/characters.md). So somebody has trained one, but there is no published recipe | [`sdxl`](../sdxl/) (mature) or [`z-image`](../z-image/)/[`flux-2`](../flux-2/); craft in [`character-lora-training`](../character-lora-training/) |
...
**Hard facts — must be exact or it breaks.** Treated as hard fact here: the architecture (9.3B single-stream DiT, 34 layers) and the Qwen3-VL-8B-Instruct encoder with its 13 concatenated hidden-state layers. The same bar covers the JSON schema and every `CaptionVerifier` rule (key order, required fields, abort-on-warning), the three sampler presets with their exact `mu`/`std`/guidance schedules, and the Magic Prompt backends. Also treated as hard fact: quantisation, the ComfyUI file layout and node names, Hugging Face gating, the licence terms, and the `Ideogram4Pipeline` / `run_inference.py` surface. **The source of truth is official**: the `ideogram-oss/ideogram4` repo, the HF model cards, the raw ComfyUI template JSON, and the licence text. These facts fail unforgivingly in two directions: a wrong quant filename 404s the download, and a misread non-commercial licence is a legal problem rather than a rendering bug. **The model is young and volatile:** quant filenames, VRAM numbers, template details and LoRA tooling all move. **Re-verify before relying on them, regardless of who said it.** LoRA support went from "does not exist" to three trainers shipping it inside fourteen weeks, and the ComfyUI quant set went from two files to three pairs in the same period.

**Craft — what actually makes a good image.** Craft here is almost entirely **JSON caption craft**. That means one subject per element, background as shell, the dual mention for shell-affixed hero objects, and committing to one value instead of hedging. It also means `bbox` strategy with its painter's-algorithm ordering, and the neutral-white-balance realism defaults. **Here the suite's usual answer does not hold.** Everywhere else, the authoritative source for craft is the community, meaning named practitioners with a track record. At three months old, Ideogram 4 has no such corpus. The self-hosting community exists: Civitai's 36 LoRA authors, the `ideogram-4-fp8` HF discussions, the ComfyUI day-0 walkthrough. It has now produced a few attributable results: the locked half-canvas identity workflow and the LoRA-plus-filter reports. But these are single well-received posts, not the repeated testing that a `[community — author]` marker usually points at. So this skill's craft rests on an **official artefact used as craft evidence**: Ideogram's open-source Magic Prompt system prompt. That is the set of instructions Ideogram gives an LLM to caption for its own model. The evidence is unusually strong for the structural rules, since they are what the model was trained against. For the aesthetic defaults it is only *informed taste*. That is why the realism section presents them as defaults rather than prohibitions. **The LoRA ecosystem is real but lopsided, and the lopsidedness has moved.** The *craft* is no longer the thin part: three trainers ship support, and a named recipe exists, even if it is one author's numbers. The *corpus* is: essentially all style work, one character-tagged LoRA with nothing written about it, no identity adapters. Read that as a statement about *tooling*: identity itself has a route that needs none of it. Inferring incapability from an empty tool shelf is the specific mistake this skill made on that axis until 2026-08-23.
...
- Where a trained LoRA plugs into a graph that loads **two** diffusion models, and at what strength, has no published example for a style LoRA. TurboTime patches the conditional model alone, but it also removes the unconditional one, so it does not answer the question for LoRAs that keep both `[flagged — re-verify]`.
- ostris's TurboTime speed LoRA is a single-author release with no independent reproduction on record `[flagged — re-verify]`.
...
**Facts dated 2026-09-09; community craft refreshed 2026-09-09.** On that date the quant filenames were re-verified against the `Comfy-Org/Ideogram-4` listing and the trainer facts against the musubi-tuner docs; the `FLASH` tier, the v3-edit-path inference and the no-adapter claim were re-checked and hold. The character-consistency and NSFW-filter craft dates from 2026-08-23. LoRA tooling, the ComfyUI quant set, the `FLASH` tier and the v3-edit-path inference move fastest — re-verify each before relying on it.
...
| `references/setup-and-workflows.md` | Read when getting the weights running or a graph wired, on your GPU or a rented pod: diffusers `Ideogram4Pipeline`, the `run_inference.py` CLI, sampler presets, resolutions, the diffusers quants (nf4/fp8) and the three ComfyUI quant pairs (fp8_scaled / nvfp4_mixed / int8_convrot) + VRAM, HF gating, the full ComfyUI node table, the TurboTime speed LoRA, GGUF status, the safety filter, and loading a LoRA you have trained |
| `references/lora-training.md` | Read before starting a training run: what the non-commercial licence permits and what it does to your LoRA, the ai-toolkit, musubi-tuner and fal trainers (and why OneTrainer's output does not load), the one named hyperparameter recipe, the unresolved question of how to caption a training set for a JSON-trained model, what the 36 Civitai LoRAs actually are (one character-tagged), and why character work here is exploratory. Cross-model craft: [`character-lora-training`](../character-lora-training/) |

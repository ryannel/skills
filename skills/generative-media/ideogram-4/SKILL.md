...
**What it costs, and what it does not settle.** Half of every render redraws a photo you already have, so a 2:1 canvas buys a square frame's worth of new image — budget the resolution accordingly, since the right half is what you keep. Nobody has measured the likeness against PuLID or a trained character LoRA, so read "consistent" as *recognisably the same person*, not *the same person to a face-embedding metric*, and reach for [`flux-2`](../flux-2/), [`z-image`](../z-image/) or [`sdxl`](../sdxl/) when it has to be exact or reused at scale. The non-commercial weights licence applies here as everywhere else self-hosted. The general lesson — why a survey of tooling silently answers a tooling-shaped question — is [`generative-media-atlas`](../generative-media-atlas/references/model-rankings.md) §3.1.

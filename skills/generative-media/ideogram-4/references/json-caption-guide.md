...
Ideogram 4 was trained **exclusively on structured JSON captions**. Plain text works, but it underperforms and trips the safety filter more often. This guide covers the full schema plus the caption-craft rules. Both come from Ideogram's own open-source Magic Prompt system prompt — the instructions Ideogram gives an LLM to write captions. That system prompt is the most authoritative prompting guidance available. The rules below are quoted or paraphrased from it, and from `docs/prompting.md` in the official repo.
...
different, much less settled problem, covered in `lora-training.md § 3`.
...
> See the realism caveat in §5: for photographic captions, the official Magic Prompt actually *folds style into prose and drops `style_description`*, and avoids stacking DSLR camera terms. Use `style_description` when you want explicit, repeatable style and palette control. Lean on prose instead for casual photoreal.
...
These are the rules that separate a mediocre caption from one that renders cleanly. They are Ideogram's own. Follow them whether you hand-write JSON or prompt an LLM to write it.

### 3.1 One subject = one element

A coherent subject — one animal, person, vehicle, building, plant, instrument, machine — is **exactly one `obj`**. Anatomical and structural parts are attributes inside that element's `desc`. They are not separate elements.
...
Identity first, then major attributes briefly, then one distinguishing detail. Each `desc` is a standalone catalog entry. Open with the subject's identity, not "the X" — that phrasing assumes the reader already saw the scene.
...
- **Shadows** — cast/drop/contact shadows, ambient occlusion. The renderer infers these on its own. Describe scene-wide light in `background` only.
- **Camera / render language** — depth of field, focus, bokeh, exposure, motion blur, lens flare, grain. Put these in `high_level_description` or `background`, and only if the user asked for them. *Exception:* viewpoint and angle (`low-angle`, `bird's-eye`, `eye-level`) are allowed in a desc.
- **Impression words** instead of physical reality — `luminous`, `radiant`, `vibrant`, `gorgeous`, `stunning`. Use observable properties instead ("cheekbone catches a small highlight", not "luminous complexion").
- **Scene-context repetition** — lighting direction, ambient surface, weather. Describe these once in `background`, not per element.
...
`background` is the scene shell. That means walls and finishes, floor/ground and its surface state, ceiling and fixtures, windows-as-architecture, atmosphere (sky, clouds, fog, dust), scene-wide ambient light, and distant out-of-focus context (horizon, blurred crowds).
...
**Ground/floor/pavement is always background — zero tolerance.** This covers floor, ground, turf, dirt, sand, asphalt, sidewalk, deck, water surface, snow, tile, hardwood, marble. This holds even if the input lists "wet pavement below" as a foreground bullet — re-classify it. Surface *state* (wet, rain-slicked, cracked, polished, muddy) and **puddles, reflections, wet patches** are part of the ground surface. They are never separate elements, even when a puddle reflects the hero.

> **The failure this prevents:** say a standing hero is the focal element, and the floor is *also* emitted as an `obj` at the bottom of the frame. The renderer then treats that floor obj as a flat 2D band rather than a receding plane, and **clips the hero's legs into it** — the figure renders half-buried, with feet and calves gone.
...
**No double-counting.** Each component lives in exactly one field. Before emitting an `obj`, scan `background` first. If it's already named there, drop the `obj`.

**No medium or post-processing in `background`** — film grain, lens flare, vignette, paper/canvas texture, halftone, brushstroke texture. These describe *how it was made*, not *what is in the scene*. Route them to `high_level_description`.
...
When the brief is sparse, populate the scene the way a real one would be. Add believable secondary subjects and micro-props by **depth layer** — a foreground crop (an out-of-focus leaf, a bowl rim) separates a photo from a postcard. **Commit to a specific cultural or regional identity** ("a Vietnamese pho stall by the rice paddies outside Hoi An", not "a Southeast Asian village"). **Built environments need text everywhere** — shop signs, sub-signs, menu boards, price labels, jar labels, posters. `text: []` is almost always wrong for a shop, stall, market, or vehicle. **Override this and stay sparse** when the brief says `minimal`, `empty`, `lonely`, `negative space`, `single subject`, or similar. Fantastical and sci-fi briefs get a populate bonus — sky drama, scale anchors, exotic architecture.
...
**The #1 bbox failure — square-on-non-square.** Because both axes run 0–1000, `[0,0,500,500]` is square *only* on a 1:1 frame. On 16:9 it's a wide rectangle; on 9:16 it's a tall one. Most bbox bugs — extra subjects, duplicates, mis-scaled objects — come from this mismatch.
...
These are the **defaults the shipped caption-expander injects for *underspecified* photographic prompts** — when medium is photo/photorealistic/selfie/real-world and nothing more is specified. They are the expander's taste, not model prohibitions. If you deliberately want a warm golden-hour or cinematic DSLR look, **specify it concretely and you'll get it.** When you *haven't* asked for a specific look, these defaults avoid the most common "AI look" tells:

- **Avoid "warm" as a bare grading adjective.** `warm light`, `warm tone`, and `warm grading` tend to trigger the amber/golden AI look. If a source is physically warm — candle, sodium lamp, sunset — name the **source** and the **light-pool color** ("amber pool from the candle") instead of grading the whole image "warm".
- **Default to the iPhone aesthetic:** phone snapshot, ambient natural or overcast daylight, **cool-neutral white balance**, accurate (not flattering) skin tones, ordinary framing. DSLR-magazine markers — creamy bokeh, telephoto compression, dramatic rim light, cinematic grade — read as AI when *unrequested*.
...
Native transparency is a first-class v4 feature — the web app can output transparent PNGs and "layerize" text. To request a cutout in a caption, set `background` to **exactly** the string `"transparent background"` (don't paraphrase). Also include the phrase `on a transparent background` in `high_level_description`.

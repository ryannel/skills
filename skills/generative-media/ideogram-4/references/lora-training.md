...
This file covers **making** a LoRA for Ideogram 4: what trains it, what the licence permits, what the community has
...
**No published hyperparameter guidance exists yet** for either path. There are no rank/alpha ladders, no
step budgets by dataset size, and no learning-rate reports from named practitioners
`[flagged — re-verify]`. fal's defaults above are the only numbers anyone has put a name to, and
they are product defaults, not a tuned recipe. Treat them as a starting point and expect to
...
suite. It follows from the model's defining fact: **it was trained exclusively on structured
...
model on a caption format it has never encountered — exactly the train/inference mismatch
...
note the `.0`; the obvious spellings return zero). So this is a real, if small, ecosystem, not
...
**Character/likeness LoRAs are essentially absent.** Adult work was absent too at that sampling: zero
of the 33 was adult-flagged. But a re-census ten days later put **~26% of 34 explicit**
`[community — Civitai baseModel census, 2026-08-23]`. So read the model-level safety filter
(`setup-and-workflows.md § 5`) as a strong prior, not a wall. The `Gray Screen bypass` entry
...
- **Character LoRAs: the training path exists, but nobody has demonstrated it.** No published
  character LoRAs exist, and there are no identity adapters (below). So treat a character run here as **exploratory work
  ...
  weights is undemonstrated. Carrying one across a few generations is not — that needs no training at
...
- **Adult/NSFW work meets the filter before it meets training.** The NSFW filter is in the weights
  and cannot be disabled (`setup-and-workflows.md § 5`). That is a model choice, not a
  ...
  others have walked, not a recipe you can follow, and expect gray screens either way

...
name: qwen-image
description: >
  Authoritative guide for the Qwen-Image family (Alibaba Qwen team) across every open variant and the hosted
  2.0/3.0 surface: Qwen-Image and Qwen-Image-2512 for text-to-image, Qwen-Image-Edit / Edit-2509 / Edit-2511
  for instruction and multi-image editing, Qwen-Image-Layered for RGBA layer decomposition, and the Lightning
  speed LoRAs. Use this whenever the user touches Qwen-Image in any way, even obliquely: choosing a variant
  (T2I vs Edit generation, which Edit for multi-image or character work, why 2.0 and 3.0 are API-only),
  ...
  VRAM from 6 GB up), setting CFG and steps correctly (true CFG 2.5–4.0 at 20–50 steps on the base, exactly 1.0
  at 4–8 steps with Lightning, and why negatives die at CFG 1), writing or fixing prompts (the Qwen2.5-VL
  encoder wants sentences; the "Picture 1: … Picture 2: …" multi-image format; the "Relight to warm backlit"
  keyword grammar; naming what must not change; quoted text rendering in English and Chinese), fixing the
  ...
  LoRAs), building a consistent character with Edit-2511 as the identity tool where no PuLID or InstantID
  exists, or checking the Apache-2.0 licence. Use this for any question about Qwen-Image in any context.
...
Qwen-Image is the Qwen team's (Alibaba) open-weights image family. Every open variant is the same **20B-parameter dual-stream MMDiT** (60 layers, 16-channel latent), conditioned by **Qwen2.5-VL 7B**. That vision-language model is the text encoder, and in the Edit variants it reads the input image as well. The VAE is a **Wan-2.1-family** autoencoder: hyperparameter-identical to Wan 2.1's, but its own class and weights, shipped as `qwen_image_vae.safetensors`. The family is bilingual (English and Chinese), specifically for rendering text. **Code and weights are both Apache 2.0.** Six open variants shipped between **2025-08-04** (Qwen-Image) and **2025-12-31** (Qwen-Image-2512). **Qwen-Image 2.0 (2026-02) and 3.0 (2026-07/08) are hosted-only**: no weights, an API node in ComfyUI, nothing to download.

**The defining trait:** the text encoder reads pictures. The same VL model that parses your prompt parses your reference images. That is why Qwen-Image-Edit is the suite's identity-without-training tool, and why in-image text survives an edit. The cost is a realism default the community calls plastic.
...
The axis that matters is **generate or edit**, then **which Edit generation**. Every open variant shares the encoder and the VAE, so the download is one transformer file per variant.
...
The mechanism is arithmetic, not folklore. At CFG 1.0 a KSampler returns the conditional prediction alone, so a negative prompt has nothing to subtract from. Negatives die in the Lightning regime because you turned guidance off, not because Qwen2.5-VL ignores them. [`z-image`](../z-image/) uses the same encoder, and its Turbo behaves the same way for the same reason. Reason from the guidance state, never from the encoder.
...
Qwen-Image runs in **ComfyUI core** with no custom nodes (2511 needs ComfyUI ≥ 0.6.0). The DiT is not a checkpoint: three loaders, plus an optional LoRA loader.
...
Qwen-Image's headline is **bilingual in-image text** and **instruction editing that preserves it**. 2509 edits the font, colour and material of existing text; 2511 folds relighting and novel viewpoints into the base. Its default *look*, though, is plastic, from three causes that get conflated `[community — nsfwVariant, Top_Buffalo1668, Square_Empress]`:
...
| Instruction editing / multi-image composition | **The headline**: 1–3 references, product placement, relight, novel angles, text edits | [`krea-2`](../krea-2/) + Identity Edit for one-sentence scene-preserving edits; [`flux-2`](../flux-2/) for its multi-reference path |
| Aesthetic range | One realism-leaning look | [`krea-2`](../krea-2/): no house look, style as a control surface |
| Commercial use under the licence | **Apache-2.0 on code and weights, no output clause, no gate** — tied with Z-Image as the least encumbered | Nothing is freer; if a job forces a gated model, read its licence section first |
...
**Timeline:** six open releases from 2025-08-04 to 2025-12-31 (dates in the selector); 2.0 announced 2026-02-10 and 3.0 Pro on the API by 2026-08, both hosted-only. "Closed" means no downloadable weights as of 2026-09-09; no Qwen statement says they never will be.

**Known limitations:** plastic skin without a realism LoRA, no identity adapter of the PuLID class, no official Nunchaku past 2509, broken 2509 LoRA compatibility on 2511, anatomy degrading above ~3 MP on Edit, no denoise ladder for img2img, and a Layered model with adoption and no craft. Publishing gates (Civitai's real-person ban, the TAKE IT DOWN Act) bind harder than capability. Owned by [`character-lora-training`](../character-lora-training/references/publishing-and-likeness.md).
...
**Hard facts — must be exact or it breaks.** This covers the architecture (20B dual-stream MMDiT, `guidance_embeds: false`, Qwen2.5-VL 7B, the Wan-2.1-family VAE), the Apache-2.0 licence, exact filenames and sizes, the Comfy-Org and Lightning repos, the node names and the CLIPLoader `qwen_image` type, every template number, the diffusers classes and version floors, the `true_cfg_scale` semantics, the trainer defaults, the Lightning fp8 correctness table, and the hosted model strings. **The source of truth is official**: the template JSON, the Comfy-Org HF trees, the six model cards and their `config.json` files, the QwenLM README and `prompt_utils.py`, the Lightning README, the trainer docs, and the Model Studio docs, read verbatim on 2026-09-09. A wrong filename 404s; a wrong shift silently degrades. The volatile ones are the packaging: Lightning and GGUF filenames, quant maintainers, template defaults, hosted pricing, and the closed status of 2.0 and 3.0. **Re-verify these before relying on them, regardless of who said it.**
...
Nothing in this skill is `[live-use]`. This lab has not trained a Qwen LoRA, and every media-lab finding a sibling cites is Krea 2 evidence.

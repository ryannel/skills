...
**Size the card against `fp8_scaled` at 13.1 GB** plus the 5.2 GB fp8 encoder — the template default, comfortable on 16–24 GB. Comfy publishes no official thresholds; below that tier the answer is community GGUF (Q2_K 4.9 GB → Q8_0 13.7 GB, via the `ComfyUI-GGUF` node, not `UNETLoader`).

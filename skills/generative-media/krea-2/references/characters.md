...
Creating a persistent, reusable character with Krea 2 means using the character-LoRA pipeline. Honestly, it is the only production path today. FLUX.2 has an adapter shortcut for this job; Krea 2 does not. Ecosystem verified 2026-07-07 (~2 weeks after open weights).
...
| Community identity-edit LoRA | **Now the standard tool, not an experiment.** `conradlocke/krea2-identity-edit`, at **v1.2** as of August 2026. It does instruction-based, identity-preserving editing. It needs its own `ComfyUI-Krea2Edit` node pack, because it uses dual conditioning (in-context VAE tokens plus image-grounded Qwen3-VL encoding) that stock nodes can't drive. It ships workflows. See §3 for the craft. `[community — NatalieCrypto, MelodicFuntasy; convergent]` |
...
5. **Train on Raw** (rank/alpha 32, LR 1e-4 starting point), **validate on Turbo** at 8-step guidance-off — this is the checkpoint you'll actually run (`lora-training.md §2, §8`).
6. **Deploy at the detailer stage** (§4).

Dataset size: the earliest recipe used 474 images plus 348 regularisation images `[community — JahJedi]`. But the now-better-replicated result is that **protocol-size datasets work**: ~50 images, 2–3k steps, trained at 1024 only, with likeness the author rates *above* their Z-Image results from the same datasets. Settings: AI-Toolkit, **LoKr factor 4, Automagic3 optimizer, sigmoid scheduling, LR 1e-4** `[community — Any_Tea_3499, r/StableDiffusion]`. The same author notes one weakness: tattoos (Ideogram beats Krea 2, which beats Z-Image, at learning them). Start at protocol size; scale up only if identity wobbles across seeds.

Multi-character LoRAs are possible but caption-sensitive. A documented failure has two characters holding up under training-style captions, then *bleeding into each other* under creative free-form prompts `[community — krigeta1, r/StableDiffusion]`. Keep each character's identity block verbatim-stable in your prompts (see §3a — description discipline is the same lever) and keep per-character detailer passes as the fallback (§5).
...
- The author's datasets were captioned lazily (trigger word only). Better captioning is expected to strengthen likeness — the well-captioned dataset in their set produced the more resilient LoRA.
...
- **Identity-edit LoRA** — `conradlocke/krea2-identity-edit` (unofficial fine-tune of Raw), **v1.2**: give it an image plus a plain-language instruction, and it edits while preserving what you didn't ask to change, *including the person*. It requires the `ComfyUI-Krea2Edit` node pack, because the LoRA's dual conditioning (in-context VAE tokens plus image-grounded Qwen3-VL encoding) needs nodes stock ComfyUI doesn't have. Ready-made workflows ship with it. It is the most direct identity tool that exists, and the obvious dataset-factory engine (§2.2).
  ...
  **Craft that emerged from a month of use** `[community — NatalieCrypto, MelodicFuntasy, r/unstable_diffusion]`:
  ...
  - **It stacks with other LoRAs.** Krea 2 + Identity Edit + an NSFW LoRA is a named production pattern for putting a *consistent* character into adult scenes `[community — Clone-Protocol-66]`, and the same stack is the community's standard undressing pipeline: the edit LoRA holds identity, the other LoRA supplies the content the base model is thin on, and neither can do the other's job. Where the base model's own reticence is what is in the way, the `krea2filterbypass3` line goes on the same stack (`lora-training.md §2b`).
...
- **Ostris edit stack** — `ostris/ComfyUI-Krea2-Ostris-Edit`: up to **3 reference images** fed through the Qwen3-VL encoder plus a model patch, driving paired-data **edit LoRAs** (a concept-edit LoRA trains in ~1,750 steps) `[community — Ostris; comfyui-wiki write-up]`. Third parties are already training on the method — for example sktksm's detail-enhancer edit LoRA, trigger "enhance this image", with its own author's caveats: experimental, weaker on horizontal aspect ratios, can shift lighting and colors `[community — sktksm]`. It is not an identity guarantee, but an edit conditioner, not a face embedder.
- **Conditioning-Rebalance** — `nova452/ComfyUI-Conditioning-Rebalance`: "IP-Adapter-like" per-layer conditioning manipulation that exploits the 12-layer text tap. It does image-reference editing plus the safety-tuning bypass; a late-June update added negative prompting for edits `[community — nova452]`. Powerful and experimental in equal measure.
...
Krea 2's prompt adherence is strong enough that an **exhaustively specific character description, repeated verbatim across scene prompts, holds a near-identical character with no training at all**. One author generates unlimited "panels" this way: a character-sheet prompt per character (age, height, build, skin, hair, eyes, wardrobe down to belt buckle and earrings), then every scene prompt re-states the full block, with a local VLM splitting a master prompt into per-scene prompts `[community — aurelm, r/comfyui "Infinite panels" workflow]`. The mechanism: the 12-layer Qwen3-VL tap rewards precise, repeated description, so identity lives in the text. The costs: prompts run very long (watch the 512-token ceiling per scene), and likeness is "near-identical", not pixel-locked. This is good for storyboards, keyframes, and dataset bootstrapping; use a LoRA when likeness must survive close-ups.
...
There is no Krea-2-specific evidence yet, so the suite's shared limits are the planning assumption. Multi-outfit LoRAs cap out around ~6 outfits before bleed. Multi-character scenes have **no regional tooling** here (no regional prompter, no attention masking like FLUX.2's), so the working method is per-face detailer passes: one pass per character, each with its own LoRA, each prompted to its own face. Character style-bleed rules follow the LoRA-stacking norms (identity ~0.8, drop style LoRAs to 0.3–0.6 when stacking). The JahJedi recipe's "holds at 0.8 under heavy style mixing" is the one measured point.
...
- **No-training identity today:** [`flux-2`](../../flux-2/) — native multi-reference (ReferenceLatent) plus PuLID is the suite's strongest adapter path.
...
This is worth stating separately, because it is now the single most common thing people do with Identity Edit, and it is not an image job at all. **Video character-replacement models want a reference that already matches the driving clip's pose and framing.** So the pipeline is: take the actual first frame of the driving video, edit it into your character with Identity Edit (or Flux Klein 9B), then hand *that* to the video model as the reference. Doing this rather than supplying a generic portrait is reported as the difference between mediocre and excellent character swaps `[community — blackmixture]`. This applies to [`scail-2`](../../scail-2/), which documents this handoff from the video side, and to [`minimax-h3`](../../minimax-h3/)'s Ref2VA video-edit mode alike.

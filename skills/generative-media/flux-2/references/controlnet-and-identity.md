...
FLUX.2 changed the block ratio: **8 double-stream + 48 single-stream** vs Flux.1's 19 double + 38 single. ControlNet injection is anchored to specific double-stream block indices, so the weight shapes are incompatible. Loading a Flux.1 ControlNet (InstantX, Shakker-Labs, XLabs) into a FLUX.2 pipeline will produce shape errors or silently corrupted output. None of the three major Flux.1 ControlNet teams (InstantX, Shakker-Labs, xinsir) had published a FLUX.2 adaptation as of June 2026.
...
The bryanmcguire integration documents `models/controlnet/` as the path, but `models/model_patches/` is what VideoX-Fun (the upstream source) specifies. If you use bryanmcguire nodes, try `models/model_patches/` first. If the loader fails, try `models/controlnet/`.

**Injection mechanism:** The apply node injects into FLUX.2 double-stream blocks 0, 2, 4, and 6 — 4 of the 8 double-stream blocks. It does this via a 260-channel control context: 128 conditioning, 4 mask, 128 inpainting. Output connects normally into the FLUX.2 FluxGuidance → BasicGuider → SamplerCustomAdvanced path.
...
PuLID (Portrait Unique Identity Locking) takes a reference portrait and locks face identity into generation. For FLUX.2, use **iFayens/ComfyUI-PuLID-Flux2**. It was the only FLUX.2-specific PuLID implementation as of June 2026.
...
- Training scripts temporarily removed due to instability in the fine-tuning path. Official weights are available, but custom-trained identity weights are not yet supported
...
For **one-off identity** (a photo of someone you can't train on), use PuLID. For **recurring character** (product persona, IP character, actor), train a LoRA. The PuLID weights are natively trained **klein-first** (`pulid_flux2_klein_v1/v2` in the `Fayens/Pulid-Flux2` repo), and calibration differs between base and distilled variants. This is a single-maintainer, fast-moving project, so re-verify current weights and limits before you install. **The full character playbook — choosing between multi-reference, PuLID, and the LoRA pipeline, the dataset factory, and multi-character scenes — is `references/characters.md`.**
...
**Core node:** `ReferenceLatent` takes a reference image, encodes it to latent via the FLUX.2 VAE, and injects it into conditioning. Chain multiple nodes for multiple references.

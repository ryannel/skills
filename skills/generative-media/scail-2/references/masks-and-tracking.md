...
Masks are SCAIL-2's control surface. This file covers them end to end: what the two masks are, how `SCAIL2ColoredMask` builds them, the `replacement_mode` contract that silently decides which job the model performs, and how identities are selected and kept stable across a shot.

It does not cover graph wiring ([`setup-and-workflows.md`](setup-and-workflows.md) §1) or identity strategy ([`characters.md`](characters.md)).
...
**There are two masks, not one, and their rules differ.** Both come from the single core node `SCAIL2ColoredMask`, built from SAM3 tracks, and both are *coloured* — the colour is a per-identity palette entry, not a soft alpha. Every quotation below is a node tooltip from the ComfyUI source `[official — PR #14373 diff]`.
...
- **`sort_by`** — `left_to_right` (default), `area`, or `none`. It fixes *"the order in which palette colors are assigned to the tracked objects (applied to both reference and pose video so each identity keeps the same color)"*. **If two characters swap identities between your reference and your clip, this is the knob to use**, not the reference ordering.

Upstream of both, your SAM3 nodes decide *what* gets tracked. Community practice is to select subjects by **point** rather than box when the frame is crowded — *"if there are more people present, [box] could lead to problems"* `[community — External_Trainer_213]`. Box selection over a crowd is the likeliest source of the reported non-target outline/glow ([`characters.md`](characters.md) §5 covers that claim).

---

## 4. Multiple references

**They go on one canvas, not in a batch.** The `reference_image` tooltip says it plainly: *"Reference image, for multiple references composite all on single image."* Identities are separated by mask colour, not by batching. Community guidance to *"feed in same number of images in the ref image batch and the mask batch"* `[community — nsfwVariant]` describes an alternate workflow, not the core node's contract. If you are on the stock graph, composite instead.
...
**Also available, and optional:** the README notes zero-shot support for **SAM3D-Body** mesh rendering as an advanced control intermediate. This is a different Meta model — body mesh, not segmentation tracking.
...
3. **Run 8 steps on the LightX2V path first.** Mode is decided long before detail is, so a cheap preview shows you the wrong-mode failure right away. The giveaway is the *source scene surviving* when you asked for a new one, or the reverse.

...
SCAIL-2 is in **ComfyUI core**. PR **#14373** brought the `WanSCAILToVideo` node, PR **#14509** added multi-reference, and there is an official tutorial at `docs.comfy.org/tutorials/video/zai/scail2`. **Load that template rather than rebuilding the graph from this description.** What follows is the topology, so you can debug it. It is not a substitute for the template's own node inventory and default widget values `[flagged — re-verify]`.
...
**No vendor VRAM figure exists in any official document.** Every row below is community-measured, and there is no official baseline to check it against `[community — no official baseline; re-verify]`. The Source column names who ran it.
...
- **fp8_scaled is the mainstream path**, and it clears 16 GB comfortably.
- **`int8_convrot` can be slower than fp8** on a mismatched CUDA/PyTorch build. The measurement above used `torch-2.8.0 + cu128`, where the kernel falls back silently instead of throwing an error. [`minimax-h3`](../../minimax-h3/) documents the same trap for CU130.
...
The two standing artefacts are the inserted character being **too bright** and **too clear** for the plate. Both are cheaper to fix downstream than to fight with settings. Re-mask the character with **SAM3** in post and adjust brightness and clarity in a normal NLE `[community — nsfwVariant]`. Re-rolling for this specific pair of problems wastes GPU time.

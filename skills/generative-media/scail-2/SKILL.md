...
SCAIL-2 runs in **ComfyUI core** — PR **#14373** (`WanSCAILToVideo`), multi-reference in PR **#14509**, tutorial at `docs.comfy.org/tutorials/video/zai/scail2`.
...
**It is a single dense DiT — one loader, one sampler chain, one LoRA per concept.** Arriving from Wan 2.2's 14B, unlearn the two-expert wiring: no expert pair, no `return_with_leftover_noise` handoff, no paired LoRA files.
...
Deltas from the stock table above.
...
Stock numbers, with an SMPL pose render substituting for the driving frames. The vendor states it **performs better at 704p** — do not run it at 512p and conclude the mode is worse. Reach for it when the source performer's clothing, build or lighting leaks through, because a pose render carries none of that.
...
8 steps, shift 1, guidance 1.0 — negatives go inert there, as on any guidance-off path. Use it to settle masks and reference framing, then re-render keepers at 40 steps.
...
4. Driving-mask background matching that flag — black for Animation, white for Replacement?
...
11. **Relighting LoRA** loaded for a Replacement shot; **Bias-Aware DPO** if hands or lip-sync matter?
...
| [`references/setup-and-workflows.md`](references/setup-and-workflows.md) | Wiring the graph, preparing inputs, or planning a long shot: node walkthrough, the step-by-step first-frame procedure, where to get a driving video, the VRAM/timing table, loading LoRAs, the chaining ladder |
...
| [`references/prompting-guide.md`](references/prompting-guide.md) | Writing the prompt — deliberately short, because the prompt is the *weakest* control surface here. What it does, what it does not, and the upstream image-edit prompt |

**No `lora-training.md`, deliberately** — see [LoRAs](#loras--you-load-them-here-you-do-not-train-them). No training path exists; [`character-lora-training`](../character-lora-training/) and [`wan-2-2`](../wan-2-2/) own that work.

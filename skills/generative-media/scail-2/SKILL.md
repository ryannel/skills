...
| Close-up facial performance: lip sync, eye and expression nuance | Weaker than Wan Animate here — see the comparison below | [`wan-2-2`](../wan-2-2/) Wan Animate |
...
| **Wan Animate** ([`wan-2-2`](../wan-2-2/)) | Transferred, with a relight LoRA | Reference-driven | None | **A task split, not a clean displacement.** Wan Animate still wins close-up facial work: lip sync, eye movement and expression nuance. SCAIL-2 wins non-human subjects, complex 3D physical action, multi-character interaction, and stability at low resolution `[community — Brie Wensleydale, same-scene comparison on X; dreamerland.ai task guide, 2026-09-09]`. For whole-body replacement it is now the default even among people who otherwise run Wan end to end `[community — blackmixture]` |
...
**Release & stability:** SCAIL-2 (arXiv 2606.10804, v3 August 2026) is new enough that everything around the weights moves weekly. The front-end **Mix Studio** turns the frame-0 rule into UI. Its telemetry is now documented, and it is narrow: two events (`App_Launched` and `Generation_Started`) sent through PostHog, held in memory only, with autocapture, session replay and person profiles disabled. There is a first-run notice and an on/off toggle under Settings → General, and an operator can point it at their own PostHog project or disable IP capture server-side `[official — BlackMixture/Mix-Studio docs/installation-and-operations.md, read 2026-09-09]`. An earlier telemetry accusation against it was never traced to a source, and the docs do not mention it, so it stays unverified rather than refuted `[flagged — re-verify]`. Do not repeat that accusation as fact. Recommend Mix Studio with the toggle in mind.
...
**Craft is what actually makes a good clip.** This covers the first-frame edit rule and its positional-hint family, zoom-crop-composite, the reverse-the-shot trick, and the "SCAIL Auto Extend" sampler. It also covers the ~161-frame guardrail, every VRAM and timing figure, front-and-back reference practice, and the task split against Wan Animate.

**The authoritative source for craft is the community**, meaning practitioners who have run this model on real shots: **blackmixture**, **nsfwVariant**, **spiderofmars**, **External_Trainer_213**, **LucidFir**, **ChairQueen**, **kayteee1995**, **DeerWoodStudios**, **develm0**, **Brie Wensleydale**. The vendor docs cover none of it. Craft claims here are stated with confidence. Ranges mean "your footage and hardware differ from the author's", not "unreliable". **The one rule is community-only and essential, which is exactly why it leads this skill.** This skill sits at the top of the suite's density band, and the reason is structural rather than stylistic. Ten named practitioners carry craft that the vendor documents nowhere, so most craft sentences need their own source.
...
- **Replacement mode altering backgrounds** and **Bernini-R's reported face-swap failure**. Each was raised publicly, each was answered by nobody, and neither has been checked against a primary source. `[community — single report; re-verify]`
- **The source of the Mix Studio telemetry accusation.** The telemetry scope itself is no longer in doubt: the project documents two PostHog events and an opt-out toggle. What remains untraced is where the original accusation came from, and whether it was ever retracted. `[flagged — re-verify]`

**Facts dated 2026-09-09; community craft refreshed 2026-09-09**. The fastest-moving parts are the ComfyUI repack filenames, the quant landscape, and the community chunking nodes, so re-verify all three. The mask contract and the licence texts are the stable core.

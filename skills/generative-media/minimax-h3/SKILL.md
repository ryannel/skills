...
description: >
  ...
  drives dialogue, sound effects and music as well as picture, the whole point of the model, plus the
  sequential-vs-simultaneous rule that causes most of the jank, timestamp placement, when to
  over-describe, and directing a performance without emotion tags); hitting the frame-count rule; picking resolution from the megapixel table; generating video
  ...
  as a single-image edit model** (one frame plus the dedicated image VAE); **replacing a character in existing
  ...
  sharpens the territory question; understanding what is *not* in the open release
  ...
  should reach for something else instead: on licence grounds, a large share of readers. Choosing between
...
MiniMax H3 is a **33B-parameter dense, single-stream omni-modal transformer** from MiniMax (licensor: **Nanonoble Pte. Ltd.**), released **2 August 2026**. It jointly understands text, images, video and audio. It generates **video with native 32 kHz stereo audio**: voice, sound effects and music are modelled together with the picture in a single forward pass, not layered on afterwards. Output runs to 24 fps, 4–15 seconds on the supported path, and up to 2K. The text encoder is the full **Qwen3-VL-32B**, with hidden states taken from its 50th layer.
...
**The defining constraint:** the licence does not cover the US, EU, UK or South Korea. This matters far more than a footnote would. The next section covers it in full. Settle it before you download anything.
...
This section comes first because it is the fact most likely to matter to you. It is also the fact the launch coverage almost always leaves out.
...
**The clause bites hardest exactly where the model is most wanted.** For explicit and adult video the community's verdict is decisive and not close: H3 sits *"far above LTX and Wan"*, and is called *"the most powerful open source model"* for the job `[community — AidenAizawa, Revolutionary-Bar766, Hearmeman98; convergent]`. That verdict does not soften the territory clause. It sharpens it, for two reasons. First, the capability leader is the one model a large share of readers cannot lawfully touch, so the temptation to skip this section is strongest for exactly the people it most concerns. Second, the obvious substitute does not exist. Of the two licence-clean alternatives below, LTX-2.5's incorporated AUP **prohibits sexually explicit generation outright, on local weights as well as the hosted service**. That leaves [`wan-2-2`](../wan-2-2/) as the only route with neither a territory nor a content gate, at a capability the same threads rank below H3. Model choice for this work, across image and video, belongs to [`generative-media-atlas`](../generative-media-atlas/)'s `references/adult-work.md`. What this skill owns is how to drive H3 once that choice is settled.

**This skill does not tell you what your legal position is.** Licence scope, the enforceability of a Hong Kong-governed click-through, and how any of it applies to you are questions for a lawyer, not a model guide. What this skill does is make sure you know the clause exists before you build on it. Finding out later is costly.

**If you are in an excluded territory**, you have two practical routes. You can use the **hosted API/app**, which runs under its own terms rather than this agreement, so check those terms. Or you can reach for a model whose licence covers you. There are two candidates, and they do not fail in the same way:

- **[`wan-2-2`](../wan-2-2/)** is **Apache 2.0**: worldwide, commercial use included, no revenue test and no content clause. It is the clean answer, and the suite's **licence-clean default**, the only one of the three with no gate on it at all. Note precisely what that claims. Wan's own skill hands the title of *default open video model* to H3 and keeps a narrower, more durable title instead. That is the right reading, and it is also why Wan is the right answer **here**. On this page you have already been ruled out by a licence, and the property you need is not popularity but the ability to ship.
- **[`ltx-2-5`](../ltx-2-5/)** also generates native audio, under the **LTX-2.x Community License Agreement** (11 August 2026). The licence text itself is **public and ungated** on GitHub; only the weights sit behind a Hugging Face contact-information gate. Use is free worldwide **below US$10M annual revenue**, aggregated across subsidiaries and affiliates. Derivatives inherit the agreement, and the licence names **LoRA adapters explicitly**, so the revenue obligation follows the LoRA to whoever ends up using it. Two clauses catch people who assume "under $10M" means clear. Attachment A ¶20 bars using the model in anything that **competes with Lightricks' own products, at any revenue level**, which binds a hobbyist as hard as a studio. And the incorporated Acceptable Use Policy **prohibits sexually explicit generation**. Its scope covers on-premises deployments, so the prohibition applies to the local weights, not only the hosted service. `[official — Lightricks/LTX-2 LICENSE.md, incorporated AUP]`

**Three models carry three different shapes of licence risk, and it is worth holding them apart.** H3 excludes whole **territories**. That is a fact about where you are, and no amount of care about your use case fixes it. LTX gates on your **company's revenue** and on **what you build**. Those are facts about your business, and they change under you as the business grows. Wan 2.2 has neither. So "which licence is most permissive" is the wrong question. The right question is which axis applies to you, because only one of these three can be satisfied by moving.
...
H3 ships as **two task-specific checkpoints**, both BF16, each bundling its own processor, tokenizer, text encoder and VAEs. A third option is a community-built hybrid of the two, and it is what most serious reference work now runs.
...
|---|---|---|
| **H3-Context-IR** | Preprocesses free-form multimodal input into the structured representation H3-Base consumes. The card calls it *"critical to the quality of the final output"* | ❌ **Hosted only.** An API reproduces the official workflow, and ComfyUI's `MiniMax H3 Context IR` partner node exposes it on credits (`references/prompting-guide.md §6`) |
...
Two consequences are worth knowing before you judge the model:
...
One more hosted tier exists that is not this model. fal.ai ships a post-trained variant it calls **H3 Max** (2026-08-28), reported at 5 s of 768p in under three seconds and top of the Artificial Analysis video board at launch. It has no open weights, so nothing in this skill applies to it beyond the prompt syntax `[community — aiweekly summary; re-verify]`.
...
- **Dialogue is written, not implied.** Give the line inside `<d>[English] …</d>`, the speaker as `(S1)`, and the delivery in prose *outside* the tag. **There is no emotion tag.** `[angry]`, `(nervous)` or `*sighs*` get spoken aloud; only a preset list of sound tags such as `(laughs)` exists. Direct a performance by describing what the face, hands and breath do, as a small arc — `references/prompting-guide.md §4`.
- **"No music" is a real instruction.** Unspecified scores get invented, so say so when you want none.
...
If you are deploying on rented GPUs, [`comfyui-on-runpod`](../comfyui-on-runpod/) owns the volume contract. H3 is the model that punishes getting it wrong hardest, because it needs **five files across three directories including two VAEs**. A missing audio VAE produces a silent video rather than an error.
...
> **The official templates ship quantised by default**: `pruned_int8_convrot` weights and the `nvfp4_awq` encoder. That choice is itself a statement about memory pressure. **`pruned` builds are inference-only.** They drop the ~13B of AdaLN-branch parameters that can be precomputed, so take a non-pruned build if you intend to fine-tune. The build matrix and VRAM figures are in `references/setup-and-workflows.md §2`.

> **The floor is lower than a 33B model suggests.** No official VRAM figure exists, and the sensible guess is wrong in the useful direction: H3 is reported running at **540p on a 3060 Ti 8 GB** with the Turbo LoRA and SageAttention. `[community — r/unstable_diffusion; single report]` Do not conclude from the parameter count that you are locked out. Try it before you rent a card. What 8 GB buys you is drafting, not keepers, because 540p is below the ~0.8 MP band where complex actions hold together.

### Do not swap in a "heretic" / abliterated text encoder

A myth spread fast enough after release that a popular Docker template shipped it: the claim that replacing the Qwen3-VL encoder with an abliterated ("heretic") build uncensors the output. **It does not.** Per the author of Heretic himself, abliteration disturbs the hidden states relative to what the transformer was trained on. That costs prompt adherence and adds artefacts. And the encoder shipped for H3 has **no refusal path in it to remove**. Its output layers are absent, which is why it is ~8 GB smaller than stock. The failure is silent: output gets subtly worse, and nothing errors. `[community — -p-e-w-, author of Heretic]`

**Abliterated models belong one stage earlier, in prompt expansion**, where a refusing LLM is a real problem. Keep the stages distinct, and watch for ComfyUI subgraphs that wire **the same LLM** into both the expander and the text-encode node. Swapping it to fix the expander silently swaps the encoder too. This is the same shape as [`krea-2`'s](../krea-2/) enhancer problem.

**Also worth knowing:** community consensus is that H3 does not meaningfully refuse anything. Reported anatomy failures are attributed to **training-data gaps, not filtering**, and supplying reference images fixes much of it. `[community — re-verify]` Here is the specific version of that, because it is what people get wrong: **base nudity prompted from text alone is *"inconsistent & low quality"*, and a nude reference image, or close-up anatomy references, is what makes an undressing sequence work at all.** `[community — nsfwVariant]` A gap in the training distribution is not something a prompt can argue with. A reference image puts the missing information into the conditioning instead. So the fix is a Ref2VA input, not a better adjective, and certainly not an encoder swap. The craft in full is in `references/prompting-guide.md §10`; the argument, quotes and cross-model lesson are in `references/setup-and-workflows.md §10`.

### diffusers

The integration is **modular-only**: `ModularPipeline.from_pretrained("MiniMaxAI/MiniMax-H3")`, with `ComponentsManager` for offloading and documented `ref2va` / `fl2va` workflows (diffusers PR #14355) `[official — diffusers docs, minimax_h3 pipeline page, 2026-08-29; re-verify]`. The model card's own snippet is still HF's generic autogenerated one showing an *image* call; ignore it and read the pipeline page.
...
| Denoise | **1.0** |
| Guidance / CFG | **none.** The graph wires `BasicGuider`, not `CFGGuider`, so it is guidance-free |
...
| LoRA weight | Turbo LoRA at **1.0** with 6–8 steps and the `beta` scheduler, per its authors; **0.8–0.85** is a widely repeated counter-tip `[contested]`. A third recipe reports the Ref2VA-distilled 4-step Turbo at **~0.1 strength with 8-step inference** as the best for facial consistency `[community — lightx2v Turbo-SLA #3; re-verify]` — see `references/lora-training.md` |

### Frame count and resolution — two rules the UI hides

**Frame count must satisfy `frames ≡ 5 (mod 17)`**. That is why the shipped default looks arbitrary: **124** (≈5 s) is `17×7 + 5`, and 362 (≈15 s) is the documented maximum. As of 2026-09-09 all three official templates default to 5 s / 124 frames; the earlier 73-frame (≈3 s) t2v/i2v default is gone `[official — Comfy-Org/workflow_templates, diffed 2026-09-09]`. Type a round number and you are off the lattice the templates guarantee. **Resolution** comes from a megapixel dial snapped to multiples of 32. Remember two values: **0.98 MP = 1344 × 768**, which is the conditioning default and what "short side 768" means, and **0.2 MP = 608 × 352**, the draft size. The formula, the valid frame counts and the full fourteen-row table are in `references/setup-and-workflows.md §3–4`.

**Resolution is a reliability dial, not just a cost dial**, and that changes how you work. Errors are reported to rise at *both* ends. Around **0.8 MP** is the sweet spot where complex actions hold together, with more small mistakes above *and* below it. `[community — nsfwVariant]` So **draft at 0.2 MP**: timing, shot order and whether the model understood the brief are all legible at 608×352 in a few minutes `[community — GrayingGamer]`. When one specific thing keeps breaking, raise steps before you raise resolution.

### FL2VA

Nothing connected → `t2va`. `first_frame`, `last_frame`, or both → `fl2va`. The non-obvious part: **a supplied frame is a strong guide, not a copy.** The model adjusts it to match the prompt rather than reproducing it. When you need the frame held, say so in the prompt (*"The scene begins with `<Picture 1>` as the first frame at 00:00.000"*) and add a retention block if that is not enough. `[community — nsfwVariant]` This is also the mode the Turbo LoRA was trained against, so it is where the speed recipes are best attested.

### Ref2VA

Ref2VA uses the same chain, plus a `match` parameter and a reference budget: ≤ 9 images, ≤ 3 video clips, ≤ 3 audio clips, ≤ 12 files, ≤ 15 s total. Reference *pixel size* acts as a weight; the allocation table is in `references/setup-and-workflows.md §5`. Two settings deltas are worth trying rather than adopting, with one report each: **`ref_image_size = MAX`** latches a face more reliably than the default `match`, and **12–15 steps** latch better than 20. `[community — single report; re-verify]` Run the hybrid checkpoint unless you have a reason not to, because plain Ref2VA is the mode with the quality gap.
...
There are four defaults, in the order they bite:

1. **It cuts.** "Random cuts I didn't ask for" is one of the most-repeated complaints about H3, and it is not a bug. Hand it several beats with no shot structure and it invents its own edit. The override is the `[Shot N]` structure with `mm:ss.000` timestamps. If you omit the timestamps, the model still chooses when to cut, which is often fine. If you supply them, you own the pacing. Never timestamp the first shot. `[community — GrayingGamer]` The corollary caught one lab out for a week: **a second shot marker or a timestamp *is* a cut**. Continuous footage is one `[Shot 1]` block with no timestamps, and once the prompt took that shape the unrequested room changes stopped, with no end-frame pinning `[live-use — media lab, Ciara hoop piece, 2026-09]`. **Where you put a timestamp is itself semantic.** A timestamp binds to the action beside it, not to the sentence, so *"at 00:03.000 she lifts her shirt, exposing her shoulders"* and *"she lifts her shirt, exposing her shoulders at 00:03.000"* schedule two different events. That is the same failure shape as the conjunction rule below: the output is plausible, just not what you asked for. `[community — nsfwVariant]`
2. **The camera drifts.** With no camera instruction, expect slow movement rather than a locked-off frame. Write `static shot` when you want no movement, and name the move when you want one, in the official grammar: one of twelve motion types plus amplitude and speed, written as a sentence inside the shot (*"the camera pushes in slowly with small amplitude"*) `[official — base prompt guide §4.3]`. Bracketed commands like `[Push in]` belong to the hosted Hailuo line and do nothing here. "The camera" left unqualified is a wandering handheld.
3. **It stages sequentially, and this is the default that costs people the most time.** Two actions joined by "and" happen one *after* the other. Use **"then"** for sequential actions and **"while"** for simultaneous ones. The practitioner who has published the most detailed H3 prompting work puts *"80% of the jank"* down to this single ambiguity, and that is credible once you see the mechanism. The prompt is read by a language model, so **narrative order in the text is the strongest temporal signal H3 has**, and English "and" carries no temporal information at all. Faced with an ambiguity, the model has to resolve it somehow. It defaults to the reading that clock time can always accommodate: one thing, then the next. The tell is subtle. You get awkward staggering that reads as bad *animation* rather than as a misread prompt, so people re-roll the seed and raise the steps instead of fixing the conjunction. `[community — nsfwVariant]`
4. **It scores the scene.** This is the sharpest of the four, and it is the rule stated above: unspecified sound is invented, not absent.
...
**The lever that fixes the rest: when H3 keeps getting one thing wrong, over-describe *that thing*.** Do not reach for a stronger adjective. Describe the mechanical path, in gratuitous detail (*"she pulls the strap off her shoulder, **then lets the dress slide straight down her legs to the floor**"*). This is worth calling out because practitioners moving over from [`wan-2-2`](../wan-2-2/) report the same tactic costing them *other* detail there, and arrive expecting to pay that price here. `[community — nsfwVariant]` They do not. H3 is adherent enough to be hand-held through its own weak spots, so lavishing three clauses on the one thing that keeps breaking buys you the fix rather than a new failure elsewhere. (Wan's own guidance is narrower than the folk version. It warns against *competing* subjects and actions, not against detail, but the practical upshot for someone switching stands.) This is also the strongest argument for writing H3 prompts by hand rather than accepting an LLM's even paragraph, which spreads its attention uniformly over exactly the parts that did not need it.

**It also acts, and the same rule applies: take the decision away from it in prose.** Left to a mood word, H3 reaches for the dominant mode of its captions, which is stock and advertising footage, and overacts. Naming the emotion makes that worse, and there is no tag syntax to name it with. What works is observable behaviour — where the eyes go, what the hands do, what the breathing does — written as a small arc and paired with a visible cue for every line of delivery, because picture and voice are rendered together `[community — Naxdy, hyukudan, TaniaDictee; convergent]`. Keep it to a sentence of arc rather than a paragraph of adjectives `[live-use — media lab, 2026-09]`. The full method, the preset sound tags that *do* exist, and the tools that mislead on this are in `references/prompting-guide.md §4`.

Seen from the other side, this is the model's real strength. A properly formatted H3 prompt reads like a **shooting script**. It is the first model in this suite where writing a prompt like a director, with blocking, beats, timings and sound, actually pays off rather than being ignored. `[community — GrayingGamer]`
...
**The default:** fix the runtime first, then add the SLA node, then the official lightx2v Turbo LoRA at 6–8 steps with the `beta` scheduler. Add Spectrum when you need more, and turn it off for a keeper, because it is an approximation and fine detail deviates. Two notes as of 2026-09-09. The **official templates now bundle the Turbo LoRA behind a `Boolean (Enable Lightning LoRA)` switch that defaults to *off***, so the stock path is still 20 steps and the 8-step branch is one click away `[official — Comfy-Org/workflow_templates, diffed 2026-09-09]`; that default has flipped at least twice, so check the switch rather than assuming. And the SLA node now has competitors — `Saganaki22/ComfyUI-sol-attn` and `Zironic/H3-Optimizations` — with **pick one attention accelerator**, never stacked, as the community rule `[community — GitHub, r/StableDiffusion; re-verify]`.
...
**The Turbo LoRA's audio tax has already been fixed in core. If you read elsewhere that the fix is still pending, that advice is out of date.** H3 runs **separate video and audio scheduling**. The sampler chain originally mishandled that once the LoRA compressed the step count: the picture stayed fine while the audio degraded. Kijai's `Comfy-Org/ComfyUI#15243` merged **2026-08-06** and shipped in **ComfyUI v0.31.0 (2026-08-08)**. It adds `ModelSamplingAV` / `ModelSamplingMiniMaxH3` with a separate **`audio_shift`** that lets stochastic samplers and low step counts carry audio correctly. `[official — Comfy-Org/ComfyUI#15243, ComfyUI v0.31.0]` So: **update ComfyUI to ≥ v0.31.0 and use the core `ModelSamplingMiniMaxH3` node** `[community — re-verify the exact stock-graph wiring]`. On an install you cannot update, larryvrh's `github.com/Larryvrh/ComfyUI-MiniMax-H3-Turbo` sampler is the fallback it was written to be. Failing both, use ~10 steps with `euler` rather than `res_multistep`. `[community — contested]`

**Contested:** whether the Turbo LoRA is worth it at all. Some named users report visible artefacts and prefer the stock path at ~15 steps. Another reports 10 steps + EasyCache beating it on both counts. One thing is not contested: speed LoRAs measurably strip micro-expression, so a performance-critical shot runs on the stock path `[community — r/StableDiffusion, three threads; convergent]`. Treat 6-step generation as a draft mode and validate keepers on the full path. `[contested]`

---

## Beyond one clip — three modes the templates do not show you

The stock templates make H3 look like a 4–15 second clip generator. It is being used for three other jobs, and each of them is the reference machinery pointed somewhere new. Each is named here with the trap that makes it fail *quietly*; the mechanics live in the references.

**Long-form: context chaining.** `Comfyui-H3--Motion-Context` (Nikodemon), usually run as the fork `ethanfel/ComfyUI-MiniMaxH3-Contex-Loop`, carries **22 frames of the previous clip** forward as context and concatenates accepted clips **with their audio**, checkpointing each one. Minute-plus continuous video is routine (~10 min per 15 s clip at 1.5 MP on a 5090). **Chain through the latent, not through decoded frames.** Decoding a frame and re-encoding it per segment darkens and degrades the video at every seam, from the pixel round-trip through the VAE plus a motion reset; a live 31 s chain confirmed it. `MiniMaxH3MotionContext`'s `context_latent` / `context_frames` / `context_audio` path avoids both. What the tool changes is what you *write*, not what you set. End every scene on a **still beat** so its last frame connects to the next scene's first, and describe every other character in every scene's prompt to stop bleed. The scene-planning rules in full are in `references/setup-and-workflows.md §7`.

**H3 as a single-image edit model.** Generate **one frame** and it becomes an image editor. It is reported to beat Krea 2 + Identity Edit, [Qwen-Image-Edit](../qwen-image/) and Flux Klein 9B on character fidelity, 3D scenes, mirrors and composition, at ~8 s per edit on a 5090 `[community — Patient_Ratio4177]`. There are two requirements, and both fail silently rather than erroring. First, **use the dedicated image VAE** (`Mamad8/MiniMax-H3-Image-VAE`). The video VAE gives soft, plausible-looking stills, and generating 5 frames to pick one does not fix it. Second, **generate exactly 1 frame**, because the image VAE grids at 5. Stock ComfyUI still clamps the node to 5 frames: the request to lift it, `Comfy-Org/ComfyUI#15644`, is **open with no linked PR as of 2026-09-09**, so the monkey-patch to `comfy_extras/nodes_minimax_h3.py` remains necessary and has to be re-applied after every update `[official — Comfy-Org/ComfyUI#15644, open]`. Settings are in `references/setup-and-workflows.md §8`. This is why [`image-production-workflows`](../image-production-workflows/) now treats a video model as a stage in an image pipeline.

**Video editing: replacing a character in existing footage.** Ref2VA will swap the person in a clip if the prompt is in the shape the official reference guide's keywords expect. Guessing that shape does not work. The finding that matters, from 400+ generations, is that **`retention_analysis` is the load-bearing block and `detailed_description` barely matters**: describing the action had no measurable effect. `[community — Darqsat]` `[video editing]` and `[audio reuse]` are **pre-trained summary keywords**, not free text, and `[audio reuse]` **re-renders** the audio rather than copying it. The worked template and failure conditions are in `references/prompting-guide.md §7`. Where exact motion transfer is the point, this is the wrong tool; see [Where MiniMax H3 sits in the suite](#where-minimax-h3-sits-in-the-suite).
...
1. **Draft** at 0.2 MP and a short frame count. Check timing, shot order, and the soundscape, which is as much a thing to iterate as the picture. This step is skippable only for a prompt you have run before.
2. **Render** at 0.98 MP (1344 × 768) with the final frame count. Not bypassable; this is the keeper.
3. **Restore or upscale**, which is *optional*, with a **temporally-aware** restorer (SeedVR2, FlashVSR), not a per-frame image upscaler. `ReDetail` is the generative alternative: it re-renders the clip through the [`ltx-2-5`](../ltx-2-5/) upscaler and invents detail rather than recovering it. Its constraints and measurements belong to [`image-production-workflows`](../image-production-workflows/). **The constraint that bites here is audio.** ReDetail refuses a silent clip, so it works on H3 output straight out of the box and fails on the same clip after any stage that dropped the track.
4. **Interpolate** for a higher frame rate. This is *optional*, and always comes **after** the restore. The ordering rule and why it holds are [`image-production-workflows`](../image-production-workflows/)'s. H3's wrinkle: interpolators are almost all picture-only, so this is usually the step that eats the audio.
...
Cross-model production craft is in [`image-production-workflows`](../image-production-workflows/), and [`wan-2-2`](../wan-2-2/) covers the same video ladder in more depth. Per-stage settings and ReDetail's VRAM and timing numbers are in `references/setup-and-workflows.md §6`.
...
| Chained clips darken and degrade at every seam | Each segment was fed a decoded frame — a pixel round-trip through the VAE, plus a motion reset at the join | Chain in latent space via `MiniMaxH3MotionContext`; for the tightest seams use FL2VA first+last interpolation, with matched environments at both endpoints or the clip morphs |
...
| A character says "angry" or "whispering" out loud | An emotion tag in the prompt — `[angry]`, `(nervous)`, `*sighs*` — is TTS syntax H3 does not have, so it is read as text | Delete it. Put the delivery in prose outside `<d>` and describe the visible behaviour; only the preset sound tags like `(laughs)` exist — `references/prompting-guide.md §4` |
| Dialogue comes out as gibberish, or the clip opens with a chirp or audio bleed | The spoken line does not fit the clip duration; the `<d>` tag also has an acknowledged bug | Draft at 0.2 MP and check the audio *fits* first; shorten the line or lengthen the clip; plain quotes in place of `<d>` reduce the chirp `[community — single report; re-verify]` |
| Performance is overacted, stock-footage-flat, or every man is a confident flirt | A named emotion pulls the model to the dominant mode of its captions; speed LoRAs also strip micro-expression | Zero emotion words; observable gaze/hand/breath cues as a small arc; stock path, no Turbo, for the shot that matters |
| Faces mush at distance, background swims when the camera moves on a wide | Framing ceiling plus wide-shot camera motion | Frame thigh-up or closer for recognition; lock the camera or keep the move small on wides |
| VRAM jumps and a previously safe job OOMs after updating ComfyUI | Reported regression on the development branch (Windows): lower system RAM, much higher VRAM for H3 graphs | Watch item, root cause unconfirmed — pin the ComfyUI version that worked until it resolves `[community — Comfy-Org/ComfyUI#16150, open 2026-09-06; single report]` |
...
5. Prompt describes **dialogue, SFX and music**, not just picture — every "and" joining two actions replaced with **"then"** or **"while"**, each speaker carrying an `(S1)` ID with delivery *outside* `<d>`, and **no emotion tags** anywhere?
...
| LoRA ecosystem and training maturity | Maturing fast — four trainers support H3, hyperparameters converge, and a stills-only character LoRA has held identity in motion in one lab's runs `[live-use]`; doctrine is still weeks old (see `references/lora-training.md`) | [`wan-2-2`](../wan-2-2/), or [`sdxl`](../sdxl/) on the image side |
| Locking a still before animating | Not an image model | [`z-image`](../z-image/) / [`flux-2`](../flux-2/) / [`krea-2`](../krea-2/) / [`sdxl`](../sdxl/) |
| **Editing a single image** | **Actually good at it** — one-frame generation with the image VAE, see above | [`krea-2`](../krea-2/) Identity Edit for scene-preserving one-sentence edits; Flux Klein 9B / [`qwen-image`](../qwen-image/) otherwise |
...
- **Whether the Turbo LoRA is worth its artefacts.** Named users disagree: some prefer it at 6–8 steps, while others prefer the stock path at ~15 steps or 10 steps + EasyCache. The 0.8–0.85 strength tip cuts across this and is not reconciled with the authors' own 1.0 recipe. `[contested]`
- **Whether `beta` should simply replace `simple` on the stock path.** The templates use `beta` only under the Turbo LoRA. A named practitioner reports it as a large improvement everywhere. If that is true, the shipped default is wrong rather than merely conservative. This is one report, easily tested, not yet corroborated. `[contested]`
- **LoRA *training*** is no longer open in the sense it was. Trainer doctrine converges (`references/lora-training.md`), and one lab has shipped a stills-only character LoRA that holds identity in motion. What remains open is narrower: whether an identity LoRA transfers FL2VA↔Ref2VA, whether clips add anything stills cannot, and character-voice training. `[flagged — re-verify]`
- **Where the delivery goes in a dialogue line.** The official guide puts it outside `<d>`; two threads report tone inside the language tag working too. Whether H3 whispers on instruction at all is likewise split. `[contested]`
...
**A third source class appears in this skill from 2026-09-09: `[live-use — media lab, <run>, <date>]`.** Those claims come from one lab's runs — the Amy and Ciara H3 LoRA ladders, and a five-scene hoop piece — with measured numbers where they exist. They are first-hand, which the community half is not, and they are also *one lab*, one identity per ladder, often n=1. Keep them distinct from consensus: a `[live-use]` number is what happened, not yet what happens.

**Facts dated 2026-09-09** (official template defaults and the Turbo switch re-diffed, ComfyUI#15644 re-checked, the two official prompt-writing guides read; architecture, licence and encoder facts unchanged since 2026-08-13); **community craft refreshed 2026-09-09** from r/StableDiffusion, GitHub prompt tools and Civitai, on top of the 2026-08-23 pass from r/StableDiffusion, r/unstable_diffusion and Civitai. The 2026-08-23 pass added the prompting craft from nsfwVariant's writeup (the sequential/simultaneous rule and its weight, timestamp placement, over-description, the ~30-step and ~0.8 MP bands, and the reference-image requirement for explicit anatomy) plus Revolutionary-Bar766's scheduler and sigma-shift settings. Those are one detailed practitioner each, corroborated in outline rather than replicated. Two hard facts were re-checked against primary sources on 2026-08-22 and are recorded above as settled rather than pending: ComfyUI PR #15243 (merged, shipped in v0.31.0) and the LTX-2.x Community License text. The tooling here is weeks old and moving weekly. Re-verify node names and recommended checkpoints before relying on them.
...
| `references/prompting-guide.md` | You are writing or fixing a prompt: soundscape vocabulary, the official dialogue syntax with speaker IDs, **emotional direction and why there is no emotion tag (§4)**, the official camera grammar and the bracket commands that belong to Hailuo instead (§5), approximating H3-Context-IR locally, worked examples **including the video-editing template (§7)**, the sequential/simultaneous rule with its mechanism, timestamp and time-budget craft (§8), prompt tooling, and **explicit/adult work and its one hard requirement (§10)** |
...
| `references/lora-training.md` | You are about to train on H3, or want to know whether you can: the trainers and their distillation handling, the converging recipe plus one lab's measured ladder (resolution plateau, rank ceiling, the `convrot8` memory trap), pruned-vs-full checkpoints, and an honest account of what is still open |

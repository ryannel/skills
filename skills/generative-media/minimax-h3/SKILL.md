...
| fps | **24**, fixed |
| Shift | no shift widget in the stock templates. `MiniMax H3 Sigma Shift` arrives with the Spectrum pack and is where the acceleration chain attaches. Once you have it, it is a quality dial as well as a wiring point: **video 12 → 15 and audio 3 → 1.5** is the reported pairing, spending more of the video schedule at high noise and less of the audio schedule there. The two move in opposite directions because H3 schedules the two streams separately — which is the same fact behind the core node's distinct `audio_shift`, and the reason one number does not serve both `[community — Revolutionary-Bar766; single report]` |

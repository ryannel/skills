...
1. **It cuts.** "Random cuts I didn't ask for" is one of the most-repeated complaints about H3, and it is not a bug. Hand it several beats with no shot structure and it invents its own edit. The override is the `[Shot N]` structure with `mm:ss.000` timestamps. If you omit the timestamps, the model still chooses when to cut, which is often fine. If you supply them, you own the pacing. Never timestamp the first shot. `[community — GrayingGamer]` **Where you put a timestamp is itself semantic.** A timestamp binds to the action beside it, not to the sentence, so *"at 00:03.000 she lifts her shirt, exposing her shoulders"* and *"she lifts her shirt, exposing her shoulders at 00:03.000"* schedule two different events. That is the same failure shape as the conjunction rule below: the output is plausible, just not what you asked for. `[community — nsfwVariant]`
2. **The camera drifts.** With no camera instruction, expect slow movement rather than a locked-off frame. Write `static shot` when you want no movement, and name the move when you want one. "The camera" left unqualified is a wandering handheld.
...
**The default:** fix the runtime first, then add the SLA node, then the official lightx2v Turbo LoRA at 6–8 steps with the `beta` scheduler. Add Spectrum when you need more, and turn it off for a keeper, because it is an approximation and fine detail deviates.
...
| LoRA ecosystem and training maturity | Maturing fast — four trainers support H3 and hyperparameters are converging, but doctrine is weeks old (see `references/lora-training.md`) | [`wan-2-2`](../wan-2-2/), or [`sdxl`](../sdxl/) on the image side |

...
**Short version: there is no settled training doctrine for H3 yet.** The model was released 2 August 2026, and this file records what is actually known eleven days later. Everything else is marked as unknown rather than filled in with plausible-sounding numbers. That is the honest state of things, and it will date quickly. Check `freshness.json` and re-verify before relying on any of it.

---

## What exists

**Speed / Turbo LoRAs are the dominant artefact so far, and they are real and usable.** The original is by **larryvrh**, and **drbaph** publishes ComfyUI-compatible conversions of it (`drbaph/MiniMax-H3-Turbo-Lora-ComfyUI`). The recipe, the settings and the audio caveat are in SKILL.md and [`setup-and-workflows.md §9`](setup-and-workflows.md). In short: 6–8 steps, the `beta` scheduler, strength 1.0, and, since ComfyUI v0.31.0, the `audio_shift` input on the core `ModelSamplingMiniMaxH3` node instead of a third-party sampler.

**As of 2026-08-22 there is an official one.** `lightx2v/Minimax-h3-Turbo` publishes `minimax_h3_fl2v_turbo_8step_v1.0_comfyui_bf16.safetensors` (4–8 step). It is a first-party ComfyUI-format release from the team that does this for the whole open-video field, so prefer it as the default. The community conversions remain in wide use, and most posted workflows still reference them. One widely-repeated tip cuts across both: **run speed LoRAs at 0.8–0.85 rather than 1.0**. This contradicts the LoRA authors' own recipe and is `[contested]`.

**Non-speed LoRAs now exist too**, which is the more meaningful signal. `Mamad8/MaxiMin-HHH-R2V-ThisIsFine` is a detail LoRA in circulation for Ref2VA work. So the claim that the ecosystem is speed-only is no longer strictly true, though by volume it still overwhelmingly is.

What that tells you: the community's first priority was **making a 33B model with a 32B encoder cheap enough to run**, not teaching it new subjects. That is the usual order of events for a large release. It means the acceleration story matured well before the customisation one did. See the four-layer acceleration stack in [`setup-and-workflows.md §9`](setup-and-workflows.md), which matured while the training story is still a blank.
...
- Which trainer supports H3, and how well. `musubi-tuner`, `diffusion-pipe` and `ai-toolkit` are the obvious candidates by track record, but their H3 support status is unverified here.
- Any hyperparameter consensus: rank, alpha, learning rate, optimiser, timestep handling. The released Turbo LoRA's own training details give a hint. It ships as versioned checkpoints, EMA and non-EMA, at steps 500/600/850, which suggests an ordinary LoRA training setup is viable. The recipe just is not published.
...
A LoRA trained against one has no defined meaning for the other. Whether weights transfer at all between them is **unknown**. They share an architecture, but they are separately trained task specialists. Assume they do not transfer until someone demonstrates otherwise.
...
Nothing H3-specific is established, so only the transferable principles from the suite apply:
...
This file is the least reliable in the skill, and it is written to be replaced. Before acting on it, check four things: whether a named trainer has shipped H3 support, whether hyperparameters have converged anywhere reproducible, whether anyone has trained audio behaviour, and whether FL2VA↔Ref2VA transfer has been tested. This is a tracked item in `freshness.json`.

...
**diffusers support is unestablished.** `Lightricks/LTX-2.5-Diffusers` exists, but it is not established which release loads it, what the pipeline class is, or whether audio decode is wired `[flagged — re-verify]`. On the hosted side, `api.ltx.io` and fal bill **per second of output**. **Pro tops out at 1080p and 10 s while Fast reaches 4K and 20 s**. **Retake, Extend, Reframe and HDR-upscale are `ltx-2-3-pro` only**. The quant matrix, timings, multi-GPU, the megapixel and API duration tables, IC-LoRA wiring and LoRA stacking are in [`references/setup-and-workflows.md`](references/setup-and-workflows.md).
...
DFR runs the distilled sigmas on the full checkpoint with the distilled LoRA. It adds **generated keyframe slots** (+16% tokens for five at 512×768 × 241) and `--temporal-upsample-rounds {0,1,2}`, each round doubling fps. **Audio is decided in stage 1 and never refined.**
...
3. **DFR temporal rounds** — `--temporal-upsample-rounds {0,1,2}`, each round doubling fps. This is the native alternative to RIFE.
4. **Detailing** — the Pixel Spatial Upscaler as `--detailing-lora`. **Two builds share that name**: `LTX-2.3-22b-IC-LoRA-Pixel-Spatial-Upscaler` (2× and 4×) and `LTX-2.5-22b-IC-LoRA-Pixel-Spatial-Upscaler`, the only 2.5-native adapter. DFR expects the 2.5 one. It is **generative, not restorative**: it "synthesizes new detail rather than faithfully preserving the reference" and is "not suited to pixel-accurate restoration, blind denoising, or compression-artifact removal."
...
Some errors fail *loudly*: a stock Gemma 4 rejected by the version check, a 2.3 monolith mixed with 2.5 split files, and NATTEN illegal memory access on an old CUDA stack. Those are covered in [`references/setup-and-workflows.md`](references/setup-and-workflows.md).
...
- The **VRAM floor** is unsettled. The documentation says 32 GB, and marketing says 16 GB and 12 GB, from the same vendor in the same week. The marketing table was publicly refuted in a 362-comment thread `[contested]`.
...
- **What ships next locally.** There is no `ExtendPipeline`, because the endpoint is `ltx-2-3-pro` only. **diffusers** is unestablished as to release, pipeline class and audio decode. There is no `ltx-2.5-pro` on Replicate `[flagged — re-verify]`.
...
- Does **2.5 beat 2.3** at all, or only at I2V? Meanwhile the ecosystem stays on 2.3: 1.58M downloads against 695k, and 168 Civitai LoRAs against 3 `[contested]`.

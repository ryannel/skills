...
This file covers the licence in detail. It walks through the clause map, Attachment A's twenty restrictions, the derivative and output duties, and the unsettled question of which text governs LTX-2.3. SKILL.md carries the four gates a reader must know before downloading. Read this file when a specific clause matters.
...
The sections below follow the agreement's own order. First the revenue grant (§§1.6, 2.1, 2.2), then Attachment A's twenty restrictions, then the incorporated AUP. After that come derivatives and redistribution (§§1.5, 3.1–3.6), outputs (§§5–6), the remaining terms (§§6, 7, 12–14), and finally the unresolved LTX-2.3 question.

## 2. The revenue threshold

SKILL.md quotes §2.1's operative sentence in full. Here are two things it does not cover.

**Aggregation (§1.6) is the part people miss.** The test is not your entity's revenue. It is the group's revenue, counted collectively across subsidiaries, affiliates and companies under common control. So a wholly-owned production company inside a large media group can be over the line based on the group's numbers.

**§2.2's carve-out** lets a Commercial Entity use the model unpaid "solely for a Non-Commercial Purpose." That covers personal hobby or research use, or "use by a Commercial Entity for testing, evaluation, or non-commercial research and development in a non-production or development environment," as long as no direct or indirect payment arises. It explicitly excludes revenue-generating activity, anything with end-user impact, and training or fine-tuning any model for commercial use.

**The breach remedy** is unpaid fees for the period of use, at "Licensor's standard commercial license fees… or, absent published standard fees, a reasonable market rate," payable within 30 days of written demand.

**The paid tier's actual terms are unreachable.** §2.1 defers pricing to a document "as will be provided by the Licensor," obtainable only by emailing `ltxv-licensing@lightricks.com`. There is no published fee schedule. The breach clause's own "absent published standard fees" wording concedes that. `[flagged — re-verify]`
...
- **Derivatives inherit the licence, and the obligation travels with them** — §3.2 and §3.5, quoted in SKILL.md. Note that §1.5's definition is generic ("any fine-tuned or adapted weights, parameters, or checkpoints derived from LTX-2.x"). **§3.5 is the only clause that names LoRA adapters**, in the parenthetical "(including any fine-tuned weights, LoRA adapters, or similar adaptations)". SKILL.md leaves out two things: §3.2's inheritance is "subject to Section 3.6," and §3.5 adds a positive duty to **notify the transferee in writing**, plus a ban on transferring "any Derivative of LTX-2.x to a Commercial Entity unless such Commercial Entity has obtained the required paid license." That is what makes an open public download hard to reconcile with the licence — you cannot vet who downloads.
...
- **Redistribution and SaaS are permitted.** §3 allows hosting "for third parties remote access purposes (e.g. software-as-a-service)" and redistributing copies "in any medium, with or without modifications." The conditions: pass §4 and Attachment A through as enforceable terms, ship the Agreement, mark modified files, and keep the notices intact.
...
So the older text is harsher on damages and lacks the evaluation carve-out, while the newer one is more explicit about adapters. A studio over the threshold planning 2.3 work should get a written answer from Lightricks rather than pick a reading. **Below the threshold, the safe default costs nothing: behave as though the January text governs.** It is strictly harsher — double liquidated damages, no evaluation carve-out — so complying with it means you also comply with the other. A hobbyist needs no legal advice to adopt it.

...
| `ltx-2.5-latent-temporal-upscaler-x2-bf16-1.0.safetensors` | `models/latent_upscale_models/` | `DFRPipeline` temporal rounds only — required as `--temporal-upsampler-path` whenever `--temporal-upscalings` is above 0 |
| `ltx-2.5-22b-distilled-lora-450-bf16.safetensors` | `models/loras/` | Two-stage pipelines that run the **full** model in stage 1 (`TI2VidTwoStages*`, A2V). **DFR stopped using it in v1.3.0** — it loads the distilled checkpoint instead |
...
| `--quantization fp8-cast` | Downcasts bf16 on the fly on any FP8-capable GPU. `fp8-scaled-mm` needs an fp8 checkpoint plus Hopper+; `nvfp4-cast` / `nvfp4-prequant` need Blackwell SM ≥ 10 and `ltx-kernels`. Since v1.3.0 (2026-08-26) NVFP4 and the CuTe DSL DiffVAE kernels also run on **Jetson Thor**, built with the arch-specific `110a` target `[official — v1.3.0 release notes]`. No measured Jetson VRAM or timing figure exists yet. Set `PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True` |
...
| Text encoding via the LTX API | The Lightricks two-stage graph exposes "(via api)" outputs, keeping the 12B Gemma out of VRAM entirely. Multi-GPU options — sequence parallel, tiled data parallel, distributed VAE decode and distributed Gemma — are in `docs/multigpu/`. DFR got its own multi-GPU runner in v1.3.0, `python -m ltx_pipelines.dfr_mgpu`, which takes the same flags as `dfr_pipeline` and runs both stages under sequence parallelism `[official — v1.3.0 release notes]` |
...
### Apple Silicon — the MLX route, not Metal PyTorch

Two things happened on the Mac side in late August, and they are easy to conflate. First, v1.3.0 fixed macOS `uv sync` so it resolves non-CUDA torch wheels `[official — v1.3.0 release notes]`. That means the repo now installs on a Mac. It does not mean the PyTorch pipelines run usefully under Metal, and no reproduced Metal benchmark has appeared. Second, and separately, the working Apple Silicon path is **MLX**: a wave of community conversions of LTX-2.5 now sits on Hugging Face, led by `mlx-community/ltx-2.5-mlx` (bf16) and `mlx-community/ltx-2.5-mlx-q8`, with q4, ditq8 and w4a8-convrot-int4 variants from several individual accounts `[community — mlx-community and others, HF, 2026-09-01; convergent]`. These are a different runtime, not a different model, so the prompt craft and the lattice carry over while the CLI flags in [§5](#5-the-ltx-pipelines-cli) do not. Treat the quantised builds the way you would any community quant: expect the q4 tier to cost detail, and read each card for its own VRAM and speed claims rather than the figures in this file. ROCm has no equivalent movement to report.
...
**v1.3.0 (2026-08-26) broke DFR's flag set, and older tutorials still carry the dead names.** `--checkpoint-path` and `--distilled-lora` are gone; the monolith path is now `--distilled-checkpoint-path`. `--temporal-upsample-rounds` became `--temporal-upscalings`, and `--num-generated-keyframes` no longer exists on DFR at all `[official — v1.3.0 release notes]`. The same release added `--compile max_video_tokens=N max_audio_tokens=N` to cap CUDA-graph input memory, dropped the `natten` requirement from `--diffvae-optimization combined_compile`, and changed the pipelines' Python return type from tuples to a named `PipelineOutput`, which breaks any script that unpacked the old tuple. Run `python -m ltx_pipelines.dfr_pipeline --help` before copying a command from a post older than that date.
...
**DFR in detail — as of `ltx-pipelines` v1.3.0 (2026-08-26).** `DFRPipeline` runs the distilled sigma schedule on the **distilled checkpoint**, the same transformer `DistilledPipeline` loads. Before v1.3.0 it ran the full checkpoint with the distilled LoRA applied; that path and its flags (`--checkpoint-path`, `--distilled-lora`) were removed, and the docs now say plainly "do not pass the full (dev) transformer" `[official — v1.3.0 release notes; docs/pipelines.md §12]`. Stage 1 generates at half resolution, plus **generated keyframe slots** on an 8-frame-border segment grid that DFR lays out itself — which is why `--num-generated-keyframes` was dropped from this pipeline. The half-resolution result is kept as a reference while video and slots are upsampled in latent space. Stage 2 re-denoises at full resolution with the 2× Pixel Spatial Upscaler as `--detailing-lora` (required; its strength is hardcoded to 0.5, so a strength you pass is ignored), conditioned on the stage-1 reference. Keyframe slots need `use_keyframes_abs_pos_embedding`, which is **2.5 only**. On 2.3 the pipeline raises an error instead of silently ignoring the request. The cost is roughly +16% tokens (≈1.35× attention) for five slots at 512×768 / 241 frames, and roughly +31% (≈1.72×) at 1088×1920 / 121 frames. **Audio comes from stage 1 only.**

**Sizes are stricter here than the 32-pixel lattice.** `--height`/`--width` are the *final* output and must divide by **64**, or by **128** with `--spatial-upscalings 2`. UHD is therefore `3840×2176`, because 2160 is off the 64-grid. `--num-frames` is the stage-1 count on the usual `8k+1` grid.

**Temporal rounds — `--temporal-upscalings {0,1,2}`, default 0.** Each round doubles *playback* fps, not duration. The canvas is temporally upsampled, cut into `2**round` tiles that meet at shared keyframes, given fresh mid-segment slots, and densified with ancestral Euler. From 121 frames you get 241 at 48 fps or 481 at 96 fps, each still about five seconds. Any round above 0 needs `--temporal-upsampler-path`. Two costs the four Euler steps hide: each round **reloads the transformer per tile**, so wall-clock grows much faster than the step count suggests, and some players treat 96 fps H.264 as 24 fps and play it in slow motion.

**The 4K path — `--spatial-upscalings 2`.** The default `1` runs stage 1 at `h/2` and stage 2 at the full `h×w`. With `2`, stage 1 drops to `h/4`, stage 2 runs at `h/2`, and a **tiled spatial epilogue** finishes the last 2×: each carry keyframe is decoded, Lanczos-stretched and re-encoded, then the 3-step detailing schedule runs over 2×2 spatial tiles with those keyframes as strength-1 conditions and frozen stage-1 audio. The docs' own rule of thumb is to reach for it when stage 2 OOMs at 4K. After denoising, DFR always keyframe-decodes, and *that* is usually the VRAM cliff rather than the denoise; `AUTO_TILING` sizes the decode tiles, and there is no decode-tile flag. Multi-GPU is `python -m ltx_pipelines.dfr_mgpu` with the same flags, both stages under sequence parallelism; v1.3.0 also fixed tiling and temporal-boundary bugs in that runner `[official — v1.3.0 release notes]`.
...
| **Replicate** | `lightricks/ltx-2.5-fast` (6.8K runs on 2026-09-09), `ltx-2.3-pro`, `ltx-2.3-fast`, `ltx-2-retake`, `audio-to-video` | **Still no `ltx-2.5-pro`** as of 2026-09-09. Pricing not read here; SKILL.md's two-bar section carries the flag |
...
| `unrecognized arguments: --temporal-upsample-rounds` (or `--distilled-lora`, `--checkpoint-path`, `--num-generated-keyframes`) on DFR | You are on `ltx-pipelines` v1.3.0+ and the command predates 2026-08-26 | `--temporal-upscalings`; `--distilled-checkpoint-path` with no LoRA; drop the keyframe count — DFR places slots itself |
| DFR rejects the dev transformer, or a `--height`/`--width` on the 32-grid | Since v1.3.0 DFR loads only a distilled, keyframe-capable checkpoint, and its output must divide by 64 (128 with `--spatial-upscalings 2`) | Pass the distilled transformer; use `3840×2176` for UHD, not `3840×2160` |

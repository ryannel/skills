...
Two more people asked the vendor account directly on release day — *"Do you know when Ref2Vid will be available?"* and *"Does it work with character references?"* — and neither question was answered `[community — RelationshipSea2360, Concheria]`. **Treat its absence as a fact about the model, not a gap in this skill.**
...
- **Pick one identifier phrase and never vary it.** Use "the woman in the yellow raincoat" at every mention. Synonym variation — "the young woman", "she", "the figure in yellow" — is exactly the kind of drift that lets the model re-roll the face.
- **Anchor on something visually large.** Hair colour, a garment, a prop, position in frame all work. A bare noun ("the man") gives the model nothing to match against, which is the same finding the H3 skill reports for its subject definitions.
...
**To combine this with multishot** — the brief most readers actually have — see [`prompting-guide.md` §12](prompting-guide.md#12-multishot-and-a-consistent-character--the-composed-path). It resolves the collision between Ingredients' two-part prompt and multishot's one-paragraph rule, names the pipeline, and gives a worked two-shot prompt.

Build the sheet as you would any character sheet: consistent lighting, neutral background, several angles, one identity per sheet. Matte the sheet or state the background explicitly, since a white-card sheet can drag its background into the shot.
...
LTX's contribution to the identity problem is real but narrow. **It is the only model here that can cut between shots inside one generation.** That means it is the only one where cross-shot identity is even a question you can ask of a single call. It is not the model to reach for when identity is the *primary* requirement.

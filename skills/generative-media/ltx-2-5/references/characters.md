...
This file owns holding a person steady: across frames within a clip, and across a **cut**, which is the harder problem and the one LTX's headline feature creates. Training the adapters mentioned here is [`lora-training.md`](lora-training.md); the prompt mechanics are [`prompting-guide.md`](prompting-guide.md).
...
Two more asked the vendor account directly on release day — *"Do you know when Ref2Vid will be available?"* and *"Does it work with character references?"* — and neither was answered `[community — RelationshipSea2360, Concheria]`. **Treat its absence as a fact about the model, not a gap in this skill.**

So the identity question here is not "how do I supply a reference" but "how do I *construct* one." Four paths exist, and only the first two are LTX-native.
...
The vendor's multishot guidance asks *you* to "reuse the same visual identifiers for recurring people or objects ('the woman in the red coat, earlier at the table, now…')." That instruction is the tell: **identity is re-anchored from the prompt at each cut rather than carried by a persistent embedding.** That is an inference from the guide's own rule, not a vendor statement.

Three practical rules follow:

- **Pick one identifier phrase and never vary it.** "The woman in the yellow raincoat" at every mention. Synonym variation — "the young woman", "she", "the figure in yellow" — is exactly the kind of drift that lets the model re-roll the face.
- **Anchor on something visually large.** Hair colour, a garment, a prop, position in frame. A bare noun ("the man") gives the model nothing to match against, which is the same finding the H3 skill reports for its subject definitions.
...
**Face drift within a clip is not fixed.** Asked directly on the 2.5 release thread whether it had been, the answer from a named user running it was a flat no `[community — Inside-Cantaloupe233, aziib]`. Shorter clips help — the conditioning signal has less distance to decay over — as does keeping the face large in frame.

**Multi-character scenes break character LoRAs.** *"Minimax prompt adherence and identity lock are miles ahead of LTX 2.3 even including character loras since multiple characters in a scene would fuck it up."* `[community — sacx05]` The community's answer is a purpose-built adapter: `MaqueAI/LTX2.3-IC-LORA-Dual-Character`, one of the more-downloaded LTX identity assets on Civitai — and, like almost everything in this ecosystem, a **2.3** asset `[community — Civitai API 2026-08-22]`.

---

## 4. The Ingredients path — the no-training default

`Lightricks/LTX-2.3-22b-IC-LoRA-Ingredients` is the closest thing LTX has to reference conditioning: you supply a **reference sheet** of panels — characters, props, locations — and the adapter carries them into the generation. It is the right first attempt because it costs no training and no licence-encumbered artefact.
...
**To combine this with multishot** — the brief most readers actually have — see [`prompting-guide.md` §12](prompting-guide.md#12-multishot-and-a-consistent-character--the-composed-path), which resolves the collision between Ingredients' two-part prompt and multishot's one-paragraph rule, names the pipeline, and gives a worked two-shot prompt.

Build the sheet as you would any character sheet — consistent lighting, neutral background, several angles, one identity per sheet — and matte or state the background explicitly, since a white-card sheet can drag its background into the shot.

---

## 5. The still-locking handoff

The cross-modality route is the same one the rest of the suite uses: **lock the character as a still in an image model, then drive I2V from it.** The image skills own that stage — [`z-image`](../../z-image/), [`flux-2`](../../flux-2/), [`krea-2`](../../krea-2/) and [`sdxl`](../../sdxl/) each carry a `characters.md` with the dataset and adapter craft, and [`character-lora-training`](../../character-lora-training/) owns what transfers across all of them.

The LTX-specific caveat is the one from SKILL.md: **a locked still and a multishot prompt pull against each other**, so you get identity *or* cuts from one generation, not both. For a multi-shot sequence with a locked character, the workable pattern is one generation per shot, each I2V-conditioned on a still of the same character and joined in post — a segment-stitching problem rather than a multishot one.
...
LTX's contribution to the identity problem is real but narrow: **it is the only model here that can cut between shots inside one generation**, which means it is the only one where cross-shot identity is even a question you can ask of a single call. It is not the model to reach for when identity is the *primary* requirement.

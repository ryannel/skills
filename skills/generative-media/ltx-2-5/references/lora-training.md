...
This file owns **making** an adapter with `ltx-trainer`. *Loading and stacking* one is [`setup-and-workflows.md §6`](setup-and-workflows.md#6-using-loras-and-ic-loras). What transfers across every model in the suite — dataset architecture, captioning doctrine, evaluation — lives in [`character-lora-training`](../../character-lora-training/); this file carries only what is specific to LTX.
...
This leads because it is the most surprising thing about LoRA work on LTX, and because finding out afterwards is expensive.

**Your LoRA is a Derivative under the licence.** §1.5 defines "Derivatives of LTX-2.x" to include "any fine-tuned or adapted weights, parameters, or checkpoints derived from LTX-2.x," and §3.5 names "any fine-tuned weights, **LoRA adapters**, or similar adaptations" explicitly. Three consequences:
...
- **Video clips versus stills.** Training on video is the only way to teach *motion*, and it costs far more VRAM and time per sample than an image dataset. Where the target is **appearance** — a face, a style, a costume — single-frame training is substantially cheaper and is the right default. That trade is the same one [`wan-2-2`](../../wan-2-2/) documents, and the reasoning transfers.
- **Silent clips are a problem in the dataset too.** The model encodes audio and video jointly; a dataset of muted footage teaches the audio branch that this content is silent. Either add a silence track deliberately (and accept that you are training silence) or use the V2A/frozen-video framing so the audio branch is not being supervised at all.
- **Caption in the model's own register.** The encoder is a Gemma 4 decoder-LM, so captions should read like the prompts in [`prompting-guide.md`](prompting-guide.md) — flowing present-tense prose, physical cues rather than labels. Tag-style captions train a dialect the inference prompt will not match.
...
**No public hyperparameter recipe exists.** Rank, alpha, learning rate and step count for a character or style LoRA on 2.5 are not published by Lightricks and were not found in a named, reproducible community write-up during this pass. Start from the sample config in the repo rather than from another model's numbers — and treat anything you find on an aggregator claiming LTX-specific rank/LR values as unattributed until you can trace it (§8 flags this and the three training unknowns beside it).

---

## 6. IC-LoRA training

An IC-LoRA is trained on **paired** data — the video plus the control signal that should drive it — which is what separates it from a style LoRA trained on video alone. In trainer terms this is the `reference` condition with video generated and audio absent.
...
Four things, all specific to training rather than to the model generally: **hyperparameters** — no published rank/alpha/LR/step recipe for a character or style LoRA on 2.5; **whether 2.5's post-training changes training behaviour** relative to 2.3, asked publicly and unanswered; **VRAM cost of training** at the validation resolution, since the repo publishes no absolute figures for inference and none for training; and **whether a 2.5-trained plain LoRA loads on 2.3**, since the IC-LoRA evidence runs the other direction only. All four resolve together, on the first named 2.5 training write-up with numbers in it. `[flagged — re-verify]`

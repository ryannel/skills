...
This suite deliberately does not restate what its vendors publish well. This file is the inventory of
what those vendors publish, what each source is *not*, and how to judge a third-party skill before
trusting it.

**Inventories read directly from the repository trees on 2026-08-23.** Vendor skill repositories add
and rename skills without notice; re-check before relying on a name.
...
**Golden paths** live under `plugins/runpod/skills/runpod/golden-paths/` — 24 numbered recipes. The
ones this suite routes to:
...
will not tell you where a text encoder must sit so `CLIPLoader` finds it, that the volume mounts at
a different root under serverless, or how to make a workflow JSON open with every node resolved.
That gap is exactly [`comfyui-on-runpod`](../../comfyui-on-runpod/)'s scope, and it is why this suite
routes rather than restates.

**One deliberate omission worth knowing:** no skill in this suite quotes GPU prices. They go stale
within weeks and are model-specific. `runpod-usage` owns the general question; `runpodctl gpu list`
...
worth borrowing regardless of where you run: **validate that a workflow has both an input node and a
...
a checkpoint, it does not tell you which one to want. They also **do nothing without the Comfy Cloud
...
The rest are LLM, Spaces, SageMaker and evaluation shaped. **`huggingface-vision-trainer` is not a
...
As of **2026-08-23**, no agent skills for their models were findable from Black Forest Labs,
...
inference repos and community write-ups of very uneven quality — which is the gap this suite fills,
and the reason its two-bar provenance discipline exists.

**Two consequences.** First, for any model in this suite, the model skill *is* the closest thing to a
canonical agent-readable source, and the official artefacts it cites are the layer beneath it.
Second, if a vendor here does publish skills later, the right move is to route to them and delete the
overlap rather than maintain a second copy — the pattern
...
enormously. Five questions, in order of how much they tell you:

1. **Does it name sources, per claim?** A skill full of numbers with no attribution is somebody's
   single workflow generalised into confident prose. It may still be right; you cannot tell, and you
...
3. **Does it separate hard facts from craft?** Filenames and licence terms fail loudly and must be
   exact; denoise bands and rank choices are ranges that depend on your data. A skill treating both
...
4. **Is it a command wrapper or is it knowledge?** Wrappers age with an API and are cheap to replace.
   Knowledge ages with an ecosystem and is expensive. Both are legitimate — know which you are
...
5. **Does it say who it is not for?** A skill that never routes you elsewhere has not thought about
   its boundary, which usually means it overlaps something else you already have.

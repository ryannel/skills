...
Adult content is a dominant use of open-weights media models, and this file treats it as a
capability axis rather than a footnote to the licence section. **It owns model choice and the
trade only.** The craft — datasets, captioning, anatomy failure modes, video specifics — belongs to
[`character-lora-training`](../../character-lora-training/) `references/nsfw-training.md`, which is
the deeper treatment and should be read alongside this.

**Two lines are absolute, and neither is a licence question**: sexual content depicting minors, and
sexual imagery of real, identifiable people without their consent. The second is not merely a
...
16 XXX), so a model's value ORs together every level present. Share of the 600 most-downloaded
LoRAs per base whose previews include **X or XXX**, measured 2026-08-23
`[official — Civitai /api/v1/models, counted this pass]`. **Reproduce it rather than trusting it** —
`python scripts/civitai_census.py --adult` re-runs exactly this measurement, and its header documents
...
- **It measures preview images, so it systematically undercounts video.** A video LoRA's preview is
  often a tame first frame or a motion demo; the LoRA can be explicit and score PG. That is almost
  certainly why the video rows sit at the bottom here while r/unstable_diffusion's top-of-month is
...
> undercount above is the most likely explanation for the video half. **Filed as a finding against
> that skill rather than silently overridden** — its qualitative claim that adult work is a dominant
> published use of open *video* models is well supported by the Reddit evidence, whatever the metric
> says.
...
unremarkable — Euler or **ER SDE**, **10 steps, guidance 1.0** — which is the point: the checkpoint
is doing the work `[community — Deep_Piece5371, convergent across posts]`. Krea 2 + Identity Edit +
...
on a **3060 Ti 8 GB** with the turbo LoRA and SageAttention, and ~5–7 min for ~10 s at 1056×608 on a
5070 Ti. **Remember gate 3**: its licence excludes the US, EU, UK and South Korea, so the community's
capability leader is one many readers cannot lawfully use — a fact the enthusiasm in those threads
does not change.
...
- **`then` for sequential, `while` for simultaneous.** The model assumes sequential when you do not
  say — *"this lack of distinction about the sequence of events causes 80% of the jank."*
- **Timestamps** in `mm:ss.000`, placed next to the action they govern; allow ≥3 s per garment for
...
- **More than 20 steps** (≈30) measurably reduces errors on complex physics, and improves audio.
...
[`ltx-2-5`](../../ltx-2-5/) is barred by its AUP universally, and the adult LTX work visible in these
threads is on **2.3** — practice, not permission, since both candidate 2.3 texts incorporate the
same AUP.
...
shift 6.5/7.0/8.0/9.0, and three retouched source images (<1/255 mean pixel delta). A 28-run seed
batch was the only axis that moved anything — **2 usable out of 28**.
...
ESRGAN restoration on the smooth-merge output changed nothing (−28.2%), which **told them the late
frames had no detail left to recover rather than being blurred** — a post-process problem ruled out
...
- **Anatomy LoRAs and character LoRAs fight, and it is unresolved.** A practitioner working through
  four published anatomy LoRAs on Wan I2V found each either failed to render or *"changes the
  character lora too much"* `[community — One-Energy5403]`. The suite's standing advice — stack a
  capability LoRA under the character LoRA — is what people are trying when this happens, so treat
  it as a live constraint rather than a solved recipe `[contested]`.
- **Same-sex and non-heteronormative scenes are a documented weak spot** across SDXL, Z-Image and
  Krea 2 alike: distorted anatomy and intrusive male anatomy in scenes that specify two women
  `[community — ricovelez; unanswered]`. This is dataset composition, not prompting, and it follows

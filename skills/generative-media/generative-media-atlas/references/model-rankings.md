...
text belong to the model skills, which are authoritative over anything here that conflicts.
...
- **The gaps are small at the top and large at the bottom.** The difference between first and second
  on realism is a pass in a pipeline; the difference between first and last on typography is
  "impossible".
- **Most axes are won by composition, not by a model.** The suite's most-repeated finding is that
  the right answer is a chain — compose where control is deepest, render where quality is highest,
  finish where the finisher is best `[community — convergent across the suite]`.
- **Rankings inherit the model skills' confidence.** Where a model skill marks a claim `[contested]`
  or `[flagged — re-verify]`, the ranking built on it carries the same weight even when the ordering
  here reads as clean.

**Convergence is the strongest signal available.** Where four independent model skills route to the
same model on one axis — as they do for [`z-image`](../../z-image/) on faces — that is a settled
...
| **SDXL photoreal finetune** | **The strength is control, not skin.** Juggernaut/RealVisXL plus the camera/film vocabulary gets a convincing frame — and then you can dictate pose, composition and identity inside it in a way no newer model matches | Base SDXL alone will not get there. 77-token window, no in-image text |
...
**The finding that matters more than the order:** realism is a *pipeline position*. The convergent
pattern across the suite is **compose and control in SDXL or Krea 2, then finish the face in Z-Image
at ~0.2 denoise** `[community — nsfwVariant, Civitai]`. Ranking the models against each other on
this axis answers a question most practitioners stopped asking. Ladder and denoise bands:
...
Two different questions hide inside "consistent characters", and they have different winners.
...
`[community — reality_comes, 402 pts, ~2026-06]`. The construction: put the reference on the **left
half of a wide canvas**, lock that half so it cannot be altered, and prompt the model to complete the
canvas as **two photos of the exact same person**, describing the new scene for the right half only.
Then cut the canvas and keep the right side.

**The mechanism is the part that generalises.** The model is composing *one* image containing two
depictions, and it can see the locked half the entire time it paints the other — so it does what any
generator does when asked to draw a character twice in one frame, and keeps them consistent. The
workflow adds no identity mechanism. It **borrows one the model already had**, converting an
...
tooling-shaped survey silently answers a tooling-shaped question. Before accepting "model X can't do
Y" anywhere in this suite — including here — ask whether the evidence is about the model or about its
ecosystem, and search for a *workflow*, not a node.
...
the path exists and gate 1 may still rule it out.
...
taking whichever lands the frame `[community — blackmixture 1796 pts, DeerWoodStudios 461 pts]` —
alongside reference-*sheet* approaches that build a 360° sheet with a video model and feed it back as
multi-image reference `[community — bstr3k, 1482 pts]`.

**Read this carefully.** Absence from a search is not evidence of inferiority; the adapters still do
things edit models cannot, and the capability ordering above stands. What the sweep establishes is
where the workflows, the troubleshooting and the help are — a real cost when you get stuck. **An
...
[`character-lora-training`](../../character-lora-training/), and the rule that decides it is
sequencing, not model choice: **re-assert identity after the last whole-image pass**, in the detailer
stage, because every refine above ~0.35 denoise erodes it.
...
axis, because the three have different winners and the trade between them is the actual decision.

### 4.1 Best likeness out of the run

One named cross-model test exists. **MesmerTools, published 2026-07-14**: the same two subjects — a
face the field finds easy and a South Asian man the field historically does not — trained across six
bases, ~30 runs, four days of GPU time on two 16 GB cards, fp8 for most models, scene-only captions,
seed-locked evaluation prompts. Their ordering, verbatim in substance:
...
benchmark.** One author, one dataset pair, one hardware tier, and the author's own caveat that the
first two evaluation rounds used prompts too basic to expose overfitting. It is nonetheless the only
controlled cross-model comparison published, and it is consistent with the one other cross-model
...
**[`z-image`](../../z-image/)** (~90 minutes per run, and useful results by step ~900 rather than
...
failed runs it takes to discover what a dataset is missing** — a missing rear angle, a jacket in
twenty of thirty images, one expression throughout. At 6 GB those failures cost nothing; at 24 GB
rented they cost real money and you take fewer of them.

### 4.3 Most documented

**[`sdxl`](../../sdxl/)**, by a wide margin: years of settled recipes, two mature trainers, and
separate Pony/Illustrious/NoobAI LoRA pools with their own conventions. Nothing else in the suite has
an answer already written for the question you are about to hit.
...
figures are exact, reached by exhausting the cursor. **Re-run it instead of trusting it once these
are weeks old:** `python scripts/civitai_census.py --pages 22` (add `--tag character` for the
character column).
...
after the two hot models, while Anima and Krea 2 own current attention.
...
- **[`z-image`](../../z-image/)'s pool is on the wrong variant for its own doctrine.** The skill's
  ...
  target **Turbo** against Base's 671. Downloading someone else's Z-Image LoRA means a Turbo LoRA;
  they generally load on Base at reduced strength, but that is a convenience, not a plan. Check the
  variant before assuming a LoRA fits your graph.
- **The FLUX.2 licence escape hatch costs you the ecosystem.** [klein] 4B is the Apache-2.0 variant
  and therefore the one to reach for under gate 1 — and it has 133 LoRAs to 9B's 653, sixteen of them
...
- **Ideogram 4's trainability win is one test against one published character LoRA.** §4.1 ranks it
  first on likeness; the host has exactly **one** character LoRA on it. Both facts are true, and
...
- **Z-Image is the pragmatic default** for a synthetic character you will render commercially: fast
  loop, Apache-2.0 on weights and outputs, and the realism axis it already owns.
...
**This ranking is why the control front-end pattern exists.** Compose in SDXL — where the pose,
depth and regional stack actually works — then render or refine in the DiT that has the quality.
Regional prompting does **not** transfer: SD-era regional tooling fails on DiTs, and core
attention-masking is the only working approach there. Per-region *LoRA* application on DiT regional
setups is unsettled `[contested]`.

---

## 6. Typography, anime and aesthetic range

**In-image typography.** [`ideogram-4`](../../ideogram-4/), with no real second — JSON captions,
`bbox` layout, text layers, transparency. FLUX.2 is "good, high variance"; Z-Image is workable for
short bilingual text; SDXL basically cannot; Krea 2 is unreliable. The trade is entirely licence:
Ideogram's open weights are non-commercial *and* have no adapter ecosystem, so the typography plate
is usually a **stage in someone else's pipeline** rather than the pipeline.
...
contest at all: a photoreal-leaning sentence-prompted generalist fights both the dialect and the
aesthetic.

**Aesthetic range without checkpoint-hopping.** [`krea-2`](../../krea-2/), deliberately built with no
house look — style references, moodboards, official style LoRAs and 1,500+ community ones. The trade
is its two taxes (soft default, muted expressions) and the ~8× per-image cost against Z-Image. SDXL
gets range too, but by *switching finetunes*, which is a different workflow.

---

## 7. Hardware and VRAM

Practitioner figures. **Several vendors publish no inference VRAM number at all**, so treat these as
...
Cleanest first. **The ranking changes depending on what leaves the building**, which is why SKILL.md
...
asks whether each may be distributed. One non-commercial rung stops the whole chain, first or last —
owned by [`image-production-workflows`](../../image-production-workflows/).
...
SDXL family on published LoRAs while [`sdxl`](../../sdxl/) still owns the *settled recipes* — years of

...
skill to use at each step, what to read in it, where the joins fail silently, and what to check
...
**Install first, then work.** Each playbook opens with its stack. Nothing installs transitively — see
[`installing-skills.md`](installing-skills.md). So a step that points at a skill you have not
...
1. **Settle the gates before step 1.** Licence, likeness and territory are SKILL.md's three gates.
   Each is cheap to check now and unrecoverable later.
...
3. **Save the intermediate of every stage.** You can only trace a bad final output back to the stage
   that broke it if that stage left evidence.
...
choose is also what you train on.** Make this choice before step 2, not after.

### Step 0 — gates

- **Likeness.** The character must not resemble an identifiable real person. Not "based on" —
  it must not *resemble*. See [`character-lora-training`](../../character-lora-training/), *Before
  anything: can you publish it?*
- **Licence.** If you will sell images, almost anything clears. If you will ship or host the
  ...
  [`sdxl`](../../sdxl/). Keep every rung of the later ladder clear too.
- **Budget.** Set a cost guard before you create anything. Use RunPod's `runpod-usage` for GPU
  ...
  backstop (batch pods: `--terminate-after` alone). A rented-GPU session must also **end with a burn
  check** — nothing `RUNNING`, volumes accounted for — before you call it done. The check and the
  reason for it are in [`comfyui-on-runpod`](../../comfyui-on-runpod/), *Cost guards that actually
  work*. Agents running these playbooks: treat the burn check as a STOP gate, not a suggestion.

### Step 1 — lock the anchor image

One image that *is* the character. Everything downstream inherits it, so put more attempts into
this step than it feels like it deserves.
...
- If the look is right but the skin is not, that is already a pipeline problem, not a model
...
- **People skip the 8-point rotation**, and that is the number-one cause of a LoRA that collapses to
  one pose. Also vary elevation, shot size, expression and lighting.
- **The chained approach is standard.** Hold the character description fixed, vary exactly one
  clause at a time, generate from the anchor with an edit model, and curate hard.
- **A video turnaround solves rotation rather than approximating it** `[community — via character-lora-training]`. Prompt a slow 360° with no
  cuts, then cut it to frames. Check the harvested model's licence first — some bar using outputs to
  train.
...
- Hyperparameters: check your model skill's `lora-training.md` first, then the cross-model starting
  ranges in [`character-lora-training`](../../character-lora-training/).
- **Save checkpoints throughout.** The best epoch is rarely the last. A run without intermediates
...
1. Training samples — free, and only good for finding roughly where the useful region is.
...
**Probe out of distribution or you have tested nothing** — try a costume, a painted style, a wide
shot where the face is small. Score likeness and prompt-adherence **separately**; they peak at
different checkpoints. Tooling: [`character-lora-training`](../../character-lora-training/)
...
ColorMatch and finish. Every stage past the first is bypassable, so preview before the expensive
passes.

**The identity rule that decides this step: re-assert the character *after* the last whole-image
pass.** Any refine above ~0.35 denoise drifts the face `[community — convergent; via image-production-workflows]`, so the detailer stage goes last. See
...
encoder, so A's sentence-prompt advice is actively wrong here. And it already knows thousands of
characters by tag, so **check whether your character needs a LoRA at all** before training one.
...
2. **Learn the dialect before anything else.** That means tag order, the `@artist` prefix
   (mandatory), the `score_*` and rating tags, and weights pushed far past SDXL norms. A
   wrong-dialect prompt reads as a bad model.
3. **Train, and do not train the LLM adapter** (`llm_adapter_lr 0`). Training it rewrites prompt
   understanding globally. That presents as "Anima got worse", not as a broken LoRA — the single
   most confusing failure here.
4. **Control is the gap.** LLLite covers lineart, depth and scribble, but there is **no pose and no
   ...
   Anima at low denoise. This is a live community pattern, and cheap at 2B.
...
2. **Write the prompt as a JSON document**, not prose. The caption schema and `bbox` layout are the
   model's actual control surface. See `references/json-caption-guide.md`.
3. **Treat the result as a plate, not a picture.** Its strongest use is as a typography or design
   layer composited into imagery another model rendered — Ideogram is weak on photoreal faces and
   has no control or identity adapters from anyone.
...
2. **Pick the video model on the constraint, not the demo:**
   - Motion and camera control, with a licence with no conditions → [`wan-2-2`](../../wan-2-2/).
   - Sound generated in the same pass → [`minimax-h3`](../../minimax-h3/) **if you are outside the
     US/EU/UK/KR**, otherwise [`ltx-2-5`](../../ltx-2-5/). LTX bars explicit content and bars
...
4. **Post, in this order: restore or upscale first, interpolate second.** Reversed, the restorer
   ...
   at either position, because it has no cross-frame consistency, so it shimmers.
...
1. **Build in the GUI, ship the API-format JSON.** Export (API) format is not the UI format. The
   `/prompt` endpoint takes the former. See [`image-production-workflows`](../../image-production-workflows/)
...
absolute lines. They are not licence questions and not capability gaps to route around. If a real
person is anywhere near the dataset, stop and read
...
4. **Do not try to fix missing anatomy with a character LoRA.** Teaching *who someone is* takes
   15–30 images. Teaching anatomy the base lacks takes 1,500+ at rank 128+. These are different
   jobs at a different scale. Stacking a capability LoRA under a character LoRA is the usual move,
   and it frequently destabilises the character — a live unsolved constraint, not a solved recipe.

...
**The chained approach is now standard**: lock an anchor image, use an edit model to generate the varied set from it, curate hard, then train. That gives coverage photography rarely does — and it sidesteps the likeness problem entirely, because the character never existed. A packaged ComfyUI implementation of the whole loop exists — **VNCCS 3.0**, with a 3D pose studio, outfit cloning across characters and per-sprite regeneration `[community — AHEKOT, r/StableDiffusion 892 pts]` — though a sprite sheet is a narrower coverage target than a training set, so curate it against the protocol above.
...
**So base-model choice dominates** — and the usual proxy for it is currently contested `[contested]`. The proxy is **the share of a base's published LoRAs that are adult-flagged**, and two measurements ten days apart disagree close to inversely on video: a 2026-08-13 sample (~100 LoRAs per base) puts **Wan 2.2 I2V highest at 90%** and Flux lowest at 28%, while a 2026-08-23 re-census (600 most-downloaded per base, reading the X/XXX bits of Civitai's `nsfwLevel` bitmask) runs **Pony 67%** down to **Wan 2.2 at 22–23%**. The likely reconciliation is that `nsfwLevel` derives from **preview images**, and a video LoRA's preview is routinely a tame first frame — so the metric undercounts video. Both tables, the methods, and the trap that the API's `nsfw` boolean is dead: **`references/nsfw-training.md` §2**. The reproducible census script lives in [`generative-media-atlas`](../generative-media-atlas/), which owns model choice.

Two things hold under either ordering. The percentages measure *ecosystem tilt*, not capability ceiling — SDXL's ~31% of a vastly larger library is more absolute material than any newer base's 60%, and SDXL is still where the purpose-built finetunes are. And **adult work is a dominant published use of open video models**, which rests on what the video community actually publishes rather than on the metric. Two models stay ruled out for reasons that are not capability at all: **Ideogram 4** by a hard model-level filter, and [`ltx-2-5`](../ltx-2-5/) by its acceptable-use policy, which prohibits explicit content universally — local weights included.

**Stacking a capability LoRA under a character LoRA is the standard answer for a base that lacks the anatomy, and it is not a reliable one `[contested]`.** A practitioner working through four published Wan I2V anatomy LoRAs found each either failed to render or *"changes the character lora too much"* `[community — One-Energy5403]` — two adapters writing the same attention weights, and the broad one wins. Run the stack test *before* committing to a base, not after the character LoRA is trained.
...
- **The adult-flagged-share ordering**: two censuses ten days apart disagree close to inversely on video, and neither method is clean — the preview-image basis of Civitai's `nsfwLevel` explains the video half of the gap but not the image half. `[contested]`
- **Whether a capability LoRA can be stacked under a character LoRA without destabilising it.** Four published anatomy LoRAs, none of which worked for one practitioner, and no recipe in reply. `[contested]`
- **Same-sex and non-heteronormative scenes** fail across SDXL, Z-Image and Krea 2 alike, and nothing in this suite answers it — the mechanism is clear (training distribution), the fix is not. `[flagged — open gap]`

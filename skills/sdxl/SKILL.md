...
**Confidence of claims in this skill.** **Primary / verified** (read from the SDXL paper arXiv 2307.01952, the Stability + ByteDance HF model cards, the licence files, and the official ComfyUI templates): architecture (2.6B UNet, dual CLIP-L + OpenCLIP-bigG, 2048-dim concat, micro-conditioning), the ~6.6B ensemble figure (from Stability's announcement), native 1024² and the resolution buckets, the stock node settings (1024², 25 steps, CFG 8, euler/normal, the 0.8 base/refiner split), the diffusers pipeline classes and ensemble code, the distillation methods, OpenRAIL++-M and Lightning's licence, and the fp16 VAE overflow. **Official-via-docs** (model-card recommendations / node docs — high confidence, spot-verify if load-bearing): the step/CFG *ranges* for base, the `CLIPTextEncodeSDXL`/`CLIPTextEncodeSDXLRefiner` fields, and the distilled-variant recommended settings. **Community / single-source** (flagged inline): VRAM thresholds, the finetune roster and their current versions/licences, the Pony/Illustrious dialect specifics, GGUF unsuitability, LoRA-tooling maturity, and the photoreal vocabulary (mined from a 2023 SD photorealism guide and recalibrated for SDXL — the *words* are CLIP-understood, but the SD1.5-era weight values were dropped).
...
| `references/checkpoints-and-loras.md` | The finetune ecosystem in depth (Juggernaut, RealVisXL, DreamShaper, Pony V6, Illustrious/NoobAI) with dialects and licence notes; the separate Pony/Illustrious LoRA pools; LoRA training with kohya_ss / OneTrainer; ControlNet and IP-Adapter model catalog; how the fast-variant LoRAs stack onto finetunes |

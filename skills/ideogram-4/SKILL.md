...
**Three surfaces, one model.** Mind the licence split:

| Surface | What it is | Commercial use? | Control |
|---|---|---|---|
| **Web app** (ideogram.ai) | The easy path — type a prompt, Magic Prompt writes the JSON for you; editor with Magic Fill, reframe, upscale, transparency | **Yes** — you own your outputs (Terms of Service); private generation on paid plans | Low–medium (UI controls) |
| **Hosted API** (api.ideogram.ai) | `Api-Key` REST endpoints; `text_prompt` (auto-Magic-Prompt) or `json_prompt` (full control) | **Yes** — you own outputs; standard no-compete clause | High (raw `json_prompt`) |
| **Self-hosted open weights** (ComfyUI / diffusers / CLI) | Download the gated weights and run them yourself — your own GPU **or a rented cloud GPU** (RunPod, Vast.ai, etc.) | **No** — weights are **Non-Commercial** (research/personal/internal only) **wherever you run them**; renting a cloud GPU does *not* grant commercial rights; commercial deployment needs a separate paid licence | Highest (every sampler knob) |

> **The licence split is the single most-misunderstood fact about this release** (see *Licence & limitations*). Code on GitHub is **Apache-2.0**; the **weights** are the **Ideogram 4 Non-Commercial Model Agreement**; **outputs you generate are yours**. Generating images for revenue-generating products via the *self-hosted weights* is explicitly outside the non-commercial grant — and that's about *purpose*, not place, so running on RunPod or any cloud GPU doesn't change it. Route commercial work through the web app or API.
...
**Confidence of claims in this skill.** **Primary / verified** (read from the GitHub repo, model source, and license): architecture, encoder, the 13 Qwen3-VL layers, the JSON schema and `CaptionVerifier` rules, the three sampler presets and their exact `mu`/`std`/guidance schedules, the Magic Prompt backends and system-prompt rules, quantisation/gating, the licence terms, and the magic-prompt API endpoint (confirmed from client source). **Official-via-docs** (developer.ideogram.ai / docs.ideogram.ai, fetched through a summariser — high confidence, spot-verify if load-bearing): the hosted API param surface, `rendering_speed` tiers, per-image pricing, and web-app feature list. **Community / single-source** (flagged in the references): exact VRAM numbers and generation times, the `nf4`-vs-`nvfp4` naming of the ComfyUI 4-bit file, GGUF availability, web-plan prices, and LoRA-training status (Ostris reported a working proof-of-concept; no production LoRA ecosystem exists yet, ~days post-launch). Independent tests place Ideogram 4 #1 among open-weight models on DesignArena and #2 on a blind designer-preference eval (behind GPT Image 2), strongest on text/typography/design and weakest on photoreal faces.
...
| `references/api-and-webapp.md` | The hosted API (generate / remix / describe / magic-prompt endpoints, full params, `json_prompt` vs `text_prompt`, resolutions, pricing & rate limits) and the web app (Magic Fill, reframe, upscale, transparency, editable text layers, style/character reference), plus the commercial-use path |
